...
#define makeid(a, b, c, d)                                                     \
    ((zlong)(((zlong)(a) << 24) | ((zlong)(b) << 16) | ((zlong)(c) << 8) |     \
             (zlong)(d)))
...
#define write_word(fp, w) (write_bytx(fp, (w) >> 8) && write_bytx(fp, (w)))
#define write_long(fp, l)                                                      \
    (write_bytx(fp, (l) >> 24) && write_bytx(fp, (l) >> 16) &&                 \
     write_bytx(fp, (l) >> 8) && write_bytx(fp, (l)))
#define write_chnk(fp, id, len) (write_long(fp, (id)) && write_long(fp, (len)))
#define write_run(fp, run) (write_byte(fp, 0) && write_byte(fp, (run)))
...
zword restore_quetzal(FILE* svf, FILE* stf)
{
    ...
    if (!read_long(svf, &tmpl) || !read_long(svf, &ifzslen) ||
    ...
    while (ifzslen > 0) {
        ...
        case ID_IFhd:
            ...
            progress |= GOT_HEADER;
            if (currlen < 13 || !read_word(svf, &tmpw)) return fatal;
            if (tmpw != z_header.release) progress = GOT_ERROR;

            for (i = H_SERIAL; i < H_SERIAL + 6; ++i) {
                if ((x = get_c(svf)) == EOF) return fatal;
                if (x != zmp[i]) progress = GOT_ERROR;
            }

            if (!read_word(svf, &tmpw)) return fatal;
            if (tmpw != z_header.checksum) progress = GOT_ERROR;
        ...
        case ID_Stks:
            ...
            for (fp = stack + STACK_SIZE, frame_count = 0; currlen > 0;
                ...
                if (tmpl & 0x10) {
                    ...
                    if (zmp[tmpl] != (zbyte)x) {
                        print_string("Save-file has wrong variable number on "
                                     "stack (possibly wrong game version?)\n");
                        return fatal;
                    }
                }
                ...
                *--sp = (zword)(fp - stack - 1); /* FP */
                ...
                if (x ^ (1 << i)) { /* Not a power of 2 */
                    print_string("Save-file uses incomplete argument lists "
            ...
            if (!(progress & GOT_MEMORY)) { /* Don't complain if two. */
                ...
                for (; currlen > 0; --currlen) {
                    ...
                    if (x == 0) { /* Start run. */
                        ...
                        --currlen;
                        if ((x = get_c(svf)) == EOF) return fatal;
                        for (; x >= 0 && i < z_header.dynamic_size; --x, ++i)
                            if ((y = get_c(stf)) == EOF)
                                return fatal;
                            else
                                zmp[i] = (zbyte)y;
        ...
        case ID_UMem:
            if (!(progress & GOT_MEMORY)) { /* Don't complain if two. */
                ...
                if (currlen == z_header.dynamic_size) {
                    ...
                    /* actually handle the problem outside if statement by
                     * skipping chunk. */
        ...
        if (skip) (void)get_c(svf); /* Skip pad byte. */
    }
...
zword save_quetzal(FILE* svf, FILE* stf)
{
    ...
    if (!write_word(svf, z_header.release)) return 0;
    for (i = H_SERIAL; i < H_SERIAL + 6; ++i)
        if (!write_byte(svf, zmp[i])) return 0;
    if (!write_word(svf, z_header.checksum)) return 0;
    ...
    for (i = n; i > 0; --i) {
        ...
        if (nargs != 0) nargs = (1 << nargs) - 1; /* Make args into bitmap. */

        /* Write the main part of the frame... */
        if (!write_long(svf, pc) || !write_byte(svf, var) ||
            !write_byte(svf, nargs) || !write_word(svf, nstk))
            return 0;

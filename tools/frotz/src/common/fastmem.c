...
void set_header_extension(int entry, zword val)
{
    ...
    if (z_header.extension_table == 0 || entry > z_header.x_table_size) return;
...
void restart_header(void)
{
    ...
    if (z_header.version == V6 && f_setup.interpreter_number == 0) {
        if (story_id == JOURNEY) {
            ...
            case 26:
                f_setup.interpreter_number = INTERP_MACINTOSH;
                break;
            case 30:
                f_setup.interpreter_number = INTERP_AMIGA;
                break;
            case 77:
                f_setup.interpreter_number = INTERP_APPLE_IIE;
                break;
            case 83:
...
void init_memory(void)
{
    ...
    if ((zmp = (zbyte huge*)zmalloc(64)) == NULL) os_fatal("Out of memory");
...
#else
    if (fread(zmp, 1, 64, story_fp) != 64) os_fatal("Story file read error");
#endif
    ...
    for (i = 0; records[i].story_id != UNKNOWN; i++) {
        if (z_header.release == records[i].release) {
            for (j = 0; j < 6; j++) {
                if (z_header.serial[j] != records[i].serial[j]) goto no_match;
            }
    ...
    if (z_header.file_size != 0) {
        ...
        if (z_header.version >= V4) story_size *= 2;
        if (z_header.version >= V6) story_size *= 2;
    ...
    for (size = 64; size < story_size; size += n) {
        if (story_size - size < 0x8000) n = (unsigned)(story_size - size);
        SET_PC(size);
        if (fread(pcp, 1, n, story_fp) != n) os_fatal("Story file read error");
    }
#endif
    ...
    if (checksum != z_header.checksum) os_fatal("Checksum failed!");
#endif
} /* init_memory */
...
void init_undo(void)
{
    ...
    if (reserve_mem != 0) {
        if ((reserved = zmalloc(reserve_mem)) == NULL) return;
    }
    ...
    undo_diff =
        malloc(((unsigned long)(z_header.dynamic_size & 0xffff) * 3) / 2 + 2);
#else
    ...
    if (reserve_mem != 0) zfree(reserved);
} /* init_undo */
...
static void free_undo(int count)
{
    ...
    if (count > undo_count) count = undo_count;
    while (count--) {
        p = first_undo;
        if (curr_undo == first_undo) curr_undo = curr_undo->next;
...
{
    if (story_fp != NULL) fclose(story_fp);
    story_fp = NULL;
...
void storeb(zword addr, zbyte value)
{
    ...
    value &= 0xff;
#endif
    if (addr >= z_header.dynamic_size) runtime_error(ERR_STORE_RANGE);
...
void z_restart(void)
{
    ...
    if (!first_restart) {
        ...
        if (fread(zmp, 1, z_header.dynamic_size, story_fp) !=
            z_header.dynamic_size)
            os_fatal("Story file read error");
    } else
        first_restart = FALSE;
    ...
    if (z_header.version != V6) {
        ...
        call(z_header.start_pc, 0, NULL, 0);
...
void z_restore(void)
{
    ...
    if (zargc != 0) {
        /* Get the file name */
        get_default_name(default_name, MAX_FILE_NAME + 1,
                         (zargc >= 3) ? zargs[2] : 0);
        ...
        if ((zargc >= 4) ? zargs[3] : 1) {
            ...
            if (new_name == NULL) goto finished;
        ...
        /* Open auxiliary file */
        if ((gfp = fopen(new_name, "rb")) == NULL) goto finished;
        ...
        if ((short)success >= 0) {
            ...
            if ((short)success > 0) {
                ...
                if (z_header.version == V3) split_window(0);
...
static void mem_undiff(zbyte* diff, long diff_length, zbyte* dest)
{
    ...
    while (diff_length) {
        ...
        if (c == 0) {
            ...
            if (!diff_length) return; /* Incomplete run */
            runlen = *diff++;
            diff_length--;
            if (runlen & 0x80) {
                if (!diff_length) return; /* Incomplete extended run */
                c = *diff++;
                diff_length--;
...
int restore_undo(void)
{
    ...
    if (f_setup.undo_slots == 0) return -1;

    /* no saved game state */
    if (curr_undo == NULL) return 0;
...
void z_save(void)
{
    ...
    if (zargc != 0) {
        ...
        /* Open auxiliary file */
        if ((gfp = fopen(new_name, "wb")) == NULL) goto finished;
...
int save_undo(void)
{
    ...
    if (undo_count == f_setup.undo_slots) free_undo(1);

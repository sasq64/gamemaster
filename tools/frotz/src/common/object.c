...
static zword object_address(zword obj)
{
    ...
    if ((obj_addr + obj_size) >= z_header.dynamic_size) {
        ...
        if (!fatal_error_given) goto illegal_obj_addr;
    }
    return obj_addr;
} /* object_address */
...
void z_get_next_prop(void)
{
    ...
    if (zargs[1] != 0) {
        ...
        if ((value & mask) != zargs[1]) runtime_error(ERR_NO_PROP);
    }
...
void z_get_prop_addr(void)
{
    ...
    if ((value & mask) == zargs[1]) {
        if (z_header.version >= V4 && (value & 0x80)) prop_addr++;

...
void stream_mssg_on(void)
{
    ...
    if (ostream_screen) screen_mssg_on();
    if (ostream_script && enable_scripting) script_mssg_on();
...
void stream_mssg_off(void)
{
    ...
    if (ostream_screen) screen_mssg_off();
    if (ostream_script && enable_scripting) script_mssg_off();
...
void z_output_stream(void)
{
    ...
    case -1:
        ...
        break;
    case 2:
        if (!ostream_script) script_open(FALSE);
        break;
    case -2:
...
{
    if (ostream_screen) screen_char(c);
    if (ostream_script && enable_scripting) script_char(c);
} /* stream_char */
...
void stream_word(const zchar* s)
{
    if (ostream_memory && !message)
        ...
        if (ostream_screen) screen_word(s);
        if (ostream_script && enable_scripting) script_word(s);
    }
} /* stream_word */
...
void stream_new_line(void)
{
    if (ostream_memory && !message)
        ...
        if (ostream_screen) screen_new_line();
        if (ostream_script && enable_scripting) script_new_line();
    }
} /* stream_new_line */
...
void z_input_stream(void)
{
    ...
    if (zargs[0] == 0 && istream_replay) replay_close();
    if (zargs[0] == 1 && !istream_replay) replay_open();
} /* z_input_stream */
...
continue_input:
    ...
    if (key == ZC_SINGLE_CLICK || key == ZC_DOUBLE_CLICK)
        if (!validate_click()) goto continue_input;

    /* Copy key to the command file */
    if (ostream_record && !istream_replay) record_write_key(key);

    /* Handle timeouts */

    if (key == ZC_TIME_OUT)
        if (direct_call(routine) == 0) goto continue_input;

    /* Handle hot keys */

    if (hot_keys && key >= ZC_HKEY_MIN && key <= ZC_HKEY_MAX) {
        if (z_header.version == V4 && key == ZC_HKEY_UNDO) goto continue_input;
        if (!handle_hot_key(key)) goto continue_input;
        return ZC_BAD;
    }
                        ...
                        bool hot_keys, bool no_scripting)
{
    ...
    if (istream_replay) screen_erase_input(buf);
...
continue_input:
    ...
    if (ostream_record && !istream_replay) record_write_input(buf, key);
    ...
    if (istream_replay) screen_write_input(buf, key);

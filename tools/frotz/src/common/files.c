...
{
    if (c == ZC_INDENT && script_width != 0) c = ' ';
    ...
    if (c > 0xff) /* Should always be false */
        c = '?';  /* Unreachable */
    if (c >= ZC_LATIN1_MIN) c = latin1_to_ibm[c - ZC_LATIN1_MIN];
    fputc(c, sfp);
    script_width++;
#else
...
void script_word(const zchar* s)
{
    ...
    if (*s == ZC_INDENT && script_width != 0) script_char(*s++);
    ...
    if (f_setup.script_cols != 0 &&
        script_width + width > f_setup.script_cols) {
        if (*s == ' ' || *s == ZC_INDENT || *s == ZC_GAP) s++;
        script_new_line();
    }
    for (i = 0; s[i] != 0; i++) {
...
void script_write_input(const zchar* buf, zchar key)
{
    ...
    if (key == ZC_RETURN) script_new_line();
} /* script_write_input */
...
zchar replay_read_input(zchar* buf)
{
    ...
    for (;;) {
        c = replay_char();
        if (c == ZC_BAD || is_terminator(c)) break;
        *buf++ = c;
    }
    *buf = 0;

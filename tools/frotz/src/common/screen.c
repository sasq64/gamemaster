...
static struct
{
    ...
    int pic1;
    int pic2;
} mapper[] = {{ZORK_ZERO, 5, 497, 498}, {ZORK_ZERO, 6, 501, 502},
              {ZORK_ZERO, 7, 499, 500}, {ZORK_ZERO, 8, 503, 504},
              {ARTHUR, 54, 170, 171},   {SHOGUN, 50, 61, 62},
              {UNKNOWN, 0, 0, 0}};
...
static zword winarg0(void)
{
    ...
    sz0 = s16(zargs[0]);
    if (z_header.version == V6 && sz0 == -3) return cwin;
    if (sz0 >= ((z_header.version == V6) ? 8 : 2)) runtime_error(ERR_ILL_WIN);
    return (zargs[0] & 0xffff);
#else
    if (z_header.version == V6 && (short)zargs[0] == -3) return cwin;
...
static zword winarg2(void)
{
    ...
    sz2 = s16(zargs[2]);
    if (zargc < 3 || sz2 == -3) return cwin;

    if (sz2 >= 8) runtime_error(ERR_ILL_WIN);

    return (zargs[2] & 0xffff);
#else
    if (zargc < 3 || (short)zargs[2] == -3) return cwin;

    if (zargs[2] >= 8) runtime_error(ERR_ILL_WIN);
...
{
    if (need_newline_at_exit) new_line();
} /* reset_screen */
...
{
    if (flag && !more_prompts) cwp->line_count = 0;
    more_prompts = flag;
} /* set_more_prompts */
...
zword get_max_width(zword win)
{
    if (z_header.version == V6) {
        if (win >= 8) runtime_error(ERR_ILL_WIN);
...
static void countdown(void)
{
    if (cwp->nl_countdown != 0) {
        if (--cwp->nl_countdown == 0) direct_call(cwp->nl_routine);
    }
} /* countdown */
...
void screen_new_line(void)
{
    ...
    if (input_window == cwin) input_redraw = TRUE;
    ...
    if (cwp->y_cursor + 2 * font_height - 1 > cwp->y_size)
        if (enable_scrolling) {
            zword y = cwp->y_pos;
            zword x = cwp->x_pos;
            os_scroll_area(y, x, y + cwp->y_size - 1, x + cwp->x_size - 1,
                           font_height);
        } else
            cwp->y_cursor = 1;
    else
...
void screen_char(zchar c)
{
    ...
    if (c == ZC_INDENT && cwp->x_cursor != cwp->left + 1) c = ' ';
...
void screen_word(const zchar* s)
{
    ...
    if (*s == ZC_INDENT && cwp->x_cursor != cwp->left + 1) screen_char(*s++);

    if (units_left() < (width = os_string_width(s))) {
        if (s[os_string_width(s) - 1] == ' ') last_char_space = TRUE;
...
#endif
        if (!last_char_space) screen_new_line();
    }
    os_display_string(s);
    cwp->x_cursor += width;
} /* screen_word */
...
void screen_write_input(const zchar* buf, zchar key)
{
    ...
    if (units_left() < (width = os_string_width(buf))) screen_new_line();

    os_display_string(buf);
    cwp->x_cursor += width;

    if (key == ZC_RETURN) screen_new_line();
} /* screen_write_input */
...
zchar console_read_input(int max, zchar* buf, zword timeout, bool continued)
{
    ...
    if (cwin == 0 &&
        units_left() + os_string_width(buf) < min_prompt_space * font_width)
        screen_new_line();

    /* Make sure the input line is visible */
    if (continued && input_redraw) screen_write_input(buf, -1);
...
static void update_attributes(void)
{
    ...
    if (story_id == ZORK_ZERO && z_header.release == 366) {
        if (cwin == 0) enable_wrapping = TRUE;
    }
...
void erase_window(zword win)
{
    ...
    os_erase_area(y, x, y + wp[win].y_size - 1, x + wp[win].x_size - 1, win);
...
void split_window(zword height)
{
    ...
    if (z_header.version != V6) height *= hi(wp[1].font_size);

    if (z_header.version <= V3) stat_height = hi(wp[7].font_size);
    ...
    sys = s16(wp[1].y_size);
    if (syc > sys) reset_cursor(1);
#else
    if ((short)wp[1].y_cursor > (short)wp[1].y_size) reset_cursor(1);
#endif
    ...
    if (syc < 1) reset_cursor(0);
#else
    if ((short)wp[0].y_cursor < 1) reset_cursor(0);
#endif

    /* Erase the upper window in V3 only */
    if (z_header.version == V3 && height != 0) erase_window(1);
...
void resize_screen(void)
{
    ...
    if (z_header.version == V6)
        ...
        if (h > 0) {
            ...
            wp[0].y_size = 1;
            scroll = wp[0].y_pos + wp[0].y_cursor - z_header.screen_height - 1;
            wp[0].y_pos = z_header.screen_height;
        }
        if (scroll > 0) {
            wp[0].y_cursor = wp[0].y_size;
            os_repaint_window(0, wp[0].y_pos + scroll, wp[0].y_pos, wp[0].x_pos,
                              wp[0].y_size, wp[0].x_size);
        }
    }
} /* resize_screen */
...
void restart_screen(void)
{
    ...
    if (z_header.version <= V3) wp[7].x_size = z_header.screen_width;
...
bool validate_click(void)
{
    if (mwin >= 0) {
        ...
        if (mouse_y < 1 || mouse_y > z_header.screen_height) return FALSE;
        if (mouse_x < 1 || mouse_x > z_header.screen_width) return FALSE;
    ...
    if (z_header.version != V6) {
        z_header.x_mouse_y =
            (z_header.x_mouse_y - 1) / z_header.font_height + 1;
...
void screen_mssg_on(void)
{
    if (cwin == 0) { /* messages in window 0 only */
        ...
        if (cwp->x_cursor != cwp->left + 1) screen_new_line();
...
void z_erase_line(void)
{
    ...
    if (--pixels == 0 || pixels > units_left()) pixels = units_left();
...
void z_get_wind_prop(void)
{
    ...
    if (story_id == ZORK_ZERO && z_header.release == 242 && zargc == 3) {
        if (zargs[2] < 16)
            ...
            storew((zword)(zargs[1] + 2),
                   os_to_true_colour(lo(wp[winarg0()].colour)));
        else if (zargs[2] == 17) {
...
void z_put_wind_prop(void)
{
    ...
    if (story_id == ZORK_ZERO && z_header.release == 242 && zargs[1] >= 16) {
        if (zargs[2] >= 16) runtime_error(ERR_ILL_WIN_PROP);
...
#ifdef TOPS20
    sz1 = s16(zargs[1]);
    os_scroll_area(y, x, y + wp[win].y_size - 1, x + wp[win].x_size - 1, sz1);
#else
    os_scroll_area(y, x, y + wp[win].y_size - 1, x + wp[win].x_size - 1,
                   (short)zargs[1]);
#endif
    if (win != cwin && !amiga_screen_model())
        os_set_colour(lo(cwp->colour), hi(cwp->colour));
...
void z_set_colour(void)
{
    ...
    if (z_header.version == V6 && !amiga_screen_model())
        ...
        if (win == 0) {
            ...
            for (i = 1; i < 8; i++) {
                ...
                if (bg2 < 16) bg2 = (bg2 == lo(wp[0].colour)) ? fg : bg;
                if (fg2 < 16) fg2 = (fg2 == lo(wp[0].colour)) ? fg : bg;
    ...
    if (win == cwin || z_header.version != V6) os_set_colour(fg, bg);
} /* z_set_colour */
...
void z_set_cursor(void)
{
    ...
    if (sy < 0) {
        if (sy == -2) cursor = TRUE;
        if (sy == -1) cursor = FALSE;
        return;
    }
#else
    if ((short)y < 0) {
        if ((short)y == -2) cursor = TRUE;
        if ((short)y == -1) cursor = FALSE;
        return;
    }
#endif
...
void z_show_status(void)
{
    ...
    if (z_header.screen_cols < 55) brief = TRUE;
    ...
    if (z_header.config & CONFIG_TIME) { /* print hours and minutes */
        ...
        print_string("Time: ");
        if (hours < 10) print_char(' ');
        print_num(hours);
        print_char(':');
        if (global2 < 10) print_char('0');
        print_num(global2);
        print_char(' ');
...
zword get_window_font(zword win)
{
    ...
    if (font == TEXT_FONT) {
        if (z_header.version != V6) {
            ...
            if (wp[win].style & FIXED_WIDTH_STYLE) font = FIXED_WIDTH_FONT;
        }
    }
    return font;
} /* get_window_font */
...
int colour_in_use(zword colour)
{
    ...
    for (i = 0; i < max; i++) {
        ...
        if (colour == fg || colour == bg) return 1;
    }
    return 0;

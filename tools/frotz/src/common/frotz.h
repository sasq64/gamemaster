...
#if defined(MSDOS_16BIT) || defined(WIN32) || defined(_WIN32) ||               \
    defined(WIN64) || defined(_WIN64) || defined(__CYGWIN__)
...
#endif
#ifndef DEFAULT_AUXILIARY_NAME
#    define DEFAULT_AUXILIARY_NAME "story.aux"
#endif
...
/*** Data access macros ***/
#ifdef TOPS20
#    define SET_BYTE(addr, v)                                                  \
        {                                                                      \
            zmp[addr] = v & 0xff;                                              \
        }
#    define LOW_BYTE(addr, v)                                                  \
        {                                                                      \
            v = zmp[addr] & 0xff;                                              \
        }
#else
...
#        define SET_WORD(addr, v)                                              \
            {                                                                  \
                *(zword _huge*)(zmp + (addr)) = bswap16(v);                    \
            }
#        define LOW_WORD(addr, v)                                              \
            {                                                                  \
                (v) = bswap16(*(zword _huge*)(zmp + (addr)));                  \
            }
#        define HIGH_WORD(addr, v)                                             \
            {                                                                  \
                (v) = bswap16(*(zword _huge*)(zmp + (addr)));                  \
...
#        define LOW_WORD(addr, v)                                              \
            {                                                                  \
                v = ((zword)(zmp[addr] & 0xff) << 8) | (zmp[addr + 1] & 0xff); \
            }
#        define HIGH_WORD(addr, v)                                             \
            {                                                                  \
                v = ((zword)(zmp[addr] & 0xff) << 8) | (zmp[addr + 1] & 0xff); \
...
#define ERR_PLAY_SOUND                                                         \
    33 /* @play_sound called without SOUND_FLAG or OLD_SOUND_FLAG set */
#define ERR_SOUND_MISSING                                                      \
    34 /* @play_sound called but picture resources are missing */
#define ERR_PICTURE_MISSING                                                    \
    35 /* @draw_picture called but picture resources are missing */

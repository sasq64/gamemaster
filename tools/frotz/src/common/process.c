...
void (*op0_opcodes[0x10])(void) = {
    z_rtrue,       z_rfalse,  z_print,      z_print_ret, z_nop,  z_save,
    z_restore,     z_restart, z_ret_popped, z_catch,     z_quit, z_new_line,
    z_show_status, z_verify,  __extended__, z_piracy};

void (*op1_opcodes[0x10])(void) = {
    z_jz,   z_get_sibling, z_get_child, z_get_parent, z_get_prop_len, z_inc,
    z_dec,  z_print_addr,  z_call_s,    z_remove_obj, z_print_obj,    z_ret,
    z_jump, z_print_paddr, z_load,      z_call_n};

void (*var_opcodes[0x40])(void) = {__illegal__,
                                   z_je,
                                   z_jl,
                                   z_jg,
                                   z_dec_chk,
...
void (*ext_opcodes[0x1d])(void) = {
    ...
    z_check_unicode, z_set_true_colour, /* spec 1.1 */
    __illegal__,     __illegal__,       z_move_window,   z_window_size,
    z_window_style,  z_get_wind_prop,   z_scroll_window, z_pop_stack,
    z_read_mouse,    z_mouse_window,    z_push_stack,    z_put_wind_prop,
    z_print_form,    z_make_menu,       z_picture_table};
...
void interpret(void)
{
    ...
    if (f_setup.script_now == 1) script_open(TRUE);
...
void call(zword routine, int argc, zword* args, int ct)
{
    ...
    if (sp - stack < 4) runtime_error(ERR_STK_OVF);
    ...
    *--sp = (zword)(argc | (ct << 12));
    ...
    if (pc >= story_size) runtime_error(ERR_ILL_CALL_ADDR);

    SET_PC(pc)
    /* Initialise local variables */
    CODE_BYTE(count)

    if (count > 15) runtime_error(ERR_CALL_NON_RTN);
    if (sp - stack < count) runtime_error(ERR_STK_OVF);
...
void ret(zword value)
{
    ...
    if (sp > fp) runtime_error(ERR_STK_UNDF);
...
#else
    if (zargs[0] != 0) call(zargs[0], zargc - 1, zargs + 1, 1);
#endif
} /* z_call_n */
...
#else
    if (zargs[0] != 0) call(zargs[0], zargc - 1, zargs + 1, 0);
#endif
    else
        store(0);
} /* z_call_s */
...
void z_jump(void)
{
    ...
    pc += (short)zargs[0] - 2;
#endif
    if (pc >= story_size) runtime_error(ERR_ILL_JUMP_ADDR);

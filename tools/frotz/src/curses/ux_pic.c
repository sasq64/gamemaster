...
static int round_div(int x, int y)
{
    ...
    if ((dblremain > y) || ((dblremain == y) && (quotient & 1))) quotient++;
    return quotient;
} /* round_div */
#endif
...
bool unix_init_pictures(void)
{
    ...
    unsigned char png_magic[8] = {0x89, 0x50, 0x4E, 0x47,
                                  0x0D, 0x0A, 0x1A, 0x0A};
    ...
    for (i = 1; i <= num_pictures; i++) {
        if (bb_load_resource(blorb_map, bb_method_Memory, &res, bb_ID_Pict,
            ...
            if (pict_info[i].type == bb_ID_PNG) {
                if (memcmp(res.data.ptr, png_magic, 8) == 0) {
                    /* Check for IHDR chunk.  If it's not there, PNG file is
                     * invalid. */
                ...
                if (memcmp(res.data.ptr, jpg_magic, 3) == 0) { /* Is it JPEG? */
                    if (memcmp(res.data.ptr + 6, jfif_name, 5) ==
                        0) { /* Look for JFIF */
                        pos = 11;
                        while (pos < res.length) {
                            pos++;
                            if (pos >= res.length) break; /* Avoid segfault */
                            if (*((unsigned char*)res.data.ptr + pos) != 0xFF)
                                continue;
                            if (*((unsigned char*)res.data.ptr + pos + 1) !=
                                0xC0)
                                continue;
                            pict_info[i].orig_width =
                                (*((unsigned char*)res.data.ptr + pos + 7) *
                                 256) +
                                *((unsigned char*)res.data.ptr + pos + 8);
                            pict_info[i].orig_height =
                                (*((unsigned char*)res.data.ptr + pos + 5) *
                                 256) +
        ...
        pict_info[i].height =
            round_div(pict_info[i].orig_height * z_header.screen_rows, y_scale);
        pict_info[i].width =
            round_div(pict_info[i].orig_width * z_header.screen_cols, x_scale);
...
bool os_picture_data(int num, int* height, int* width)
{
    ...
    *width = 0;
    ...
    if ((index = z_num_to_index(num)) == -1) return FALSE;
...
static void safe_scrnset(int y, int x, int ch, int n)
{
    if ((y < z_header.screen_rows) && (x < z_header.screen_cols)) {
        move(y, x);
        if (x + n > z_header.screen_cols) n = z_header.screen_cols - x;
        while (n--)
            addch(ch);
    }
} /* safe_scrnset */
...
void os_draw_picture(int num, int row, int col)
{
    ...
    if (!os_picture_data(num, &height, &width) || !width || !height) return;
    col--, row--;
...
int os_peek_colour(void)
{
    if (u_setup.color_enabled) {
        ...
        return (inch() & A_REVERSE) ? z_header.default_foreground

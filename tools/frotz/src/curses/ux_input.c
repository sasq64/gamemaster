...
static bool timeout_left(struct timeval* diff)
{
    ...
    if (global_timeout.tv_sec == 0) return false;
    ...
    if (diff->tv_sec < 0) diff->tv_sec = diff->tv_usec = 0;
    return true;
} /* timeout_left */
...
void os_tick()
{
    ...
    if (f_setup.sound) {
        if (ux_sem_trywait(&sound_done) == 0) end_of_sound();
    }
#endif
} /* os_tick */
...
static int unix_read_char(int extkeys)
{
    ...
    while (1) {
        ...
        if (terminal_resized) continue;
        switch (sel) {
        case -1:
            if (errno != EINTR) os_fatal(strerror(errno));
            continue;
        case 0:
            if (t_left == NULL)
                /*
        ...
        if ((c >= ZC_ASCII_MIN && c <= ZC_ASCII_MAX) ||
            (!u_setup.plain_ascii && c >= ZC_LATIN1_MIN && c <= ZC_LATIN1_MAX))
            return c;
...
static void unix_add_to_history(zchar* str)
{
    ...
    if (*history_next != NULL) free(*history_next);
                   ...
                   int continued)
{
    ...
    for (;;) {
        int x2, max;
        if (scrpos >= width) scrpos = width - 1;
        move(y, x + scrpos);
        ...
        if (len > max) {
            len = max;
            if (scrpos > len) scrpos = len;
            if (searchpos > len) searchpos = len;
        }
        switch (ch) {
        ...
        case KEY_END:
            ...
            continue;
        case ZC_WORD_RIGHT:
            if (scrpos < len) scrpos = end_of_next_word(scrpos, buf, len);
            continue;
        case ZC_WORD_LEFT:
            if (scrpos > 0) scrpos = start_of_prev_word(scrpos, buf);
            continue;
        ...
        if (is_terminator(ch)) {
            buf[len] = '\0';
            if (ch == ZC_RETURN) unix_add_to_history(buf);
...
char* os_read_file_name(const char* default_name, int flag)
{
    ...
    if (f_setup.restricted_path) {
        ...
        if (strlen(tempname) > 1) return NULL;
    }
    ...
    if (f_setup.restricted_path != NULL) {
        ...
        if (file_name[strlen(file_name) - 1] != PATH_SEPARATOR) {
            strncat(file_name, path_separator,
                    FILENAME_MAX - strlen(file_name) - 2);
        }
    ...
    if ((flag == FILE_SAVE || flag == FILE_SAVE_AUX || flag == FILE_RECORD ||
        ...
        if (tolower(answer[0] != 'y')) return NULL;
    }

...
typedef struct sound_stream
{
    ...
    int (*process)(struct sound_stream* self, float* outl, float* outr,
                   unsigned samples);
...
typedef struct
{
    int (*process)(sound_stream_t* self, float* outl, float* outr,
                   unsigned samples);
...
static int32_t make_id(uint8_t a, uint8_t b, uint8_t c, uint8_t d)
{
    return (a << 24) | (b << 16) | (c << 8) | d;
} /* make_id */

static int get_type(int magic)
{
    /*fprintf(audio_log, "magic = %x\n", magic);*/
    if (magic == make_id('F', 'O', 'R', 'M')) return FORM;
    if (magic == make_id('M', 'O', 'D', ' ')) return MOD;
    if (magic == make_id('O', 'G', 'G', 'V')) return OGGV;
    return -1;
} /* get_type */

static float limit(float mn, float mx, float v)
{
    if (v < mn) return mn;
    if (v > mx) return mx;
    return v;
...
static resampler_t* resampler_init(int sample_rate_input)
{
    ...
    rsmp->src_data.src_ratio =
        frotz_audio.sample_rate * 1.0f / sample_rate_input;
...
static void resampler_cleanup(resampler_t* rsmp)
{
...
static int resampler_step(resampler_t* rsmp, float* block)
{
    /*Always stereo*/
     ...
     */
    if (rsmp->src_data.input_frames == 0) rsmp->src_data.data_in = rsmp->input;
    int g_out = rsmp->src_data.output_frames_gen;
    rsmp->src_data.data_out += 2 * g_out;
    rsmp->src_data.output_frames -= g_out;

    if (rsmp->src_data.output_frames == 0) return 0;
    return 1;
} /* resampler_step */

static void resampler_consume(resampler_t* rsmp)
{
    rsmp->src_data.data_out = rsmp->output;
    rsmp->src_data.output_frames = frotz_audio.buffer_size;
} /* resampler_consume */
...
static int process_mod(sound_stream_t* self_, float* outl, float* outr,
                       unsigned samples)
{
...
static void cleanup_mod(sound_stream_t* s)
{
...
{
    sound_stream_mod_t* stream =
        (sound_stream_mod_t*)calloc(sizeof(sound_stream_mod_t), 1);
    long size;
    ...
    stream->shortbuffer =
        (int16_t*)calloc(frotz_audio.buffer_size, sizeof(short) * 2);
...
static int process_aiff(sound_stream_t* self_, float* outl, float* outr,
                        unsigned samples)
{
    ...
    while (needs_data) {
        int inf =
            sf_readf_float(self->sndfile, self->floatbuffer, remaining_samples);
        if (self->sf_info.channels == 1) {
            for (i = 0; i < inf; ++i) {
                self->rsmp
                    ->scratch[2 * i + 0 + 2 * (samples - remaining_samples)] =
                    self->floatbuffer[i];
                self->rsmp
                    ->scratch[2 * i + 1 + 2 * (samples - remaining_samples)] =
                    self->floatbuffer[i];
            }
        ...
        if (inf < (int)remaining_samples) {
            if (self->repeats < 255) self->repeats--;
            if (self->repeats > 0) {
                /*
...
static void cleanup_aiff(sound_stream_t* s)
{
...
static sf_count_t mem_snd_read(void* ptr_, sf_count_t size, void* datasource)
{
    ...
    while (to_read > 0) {
        did_read =
            pread(fileno(fr->file), ptr, size, fr->pos + fr->base_offset);
        if (did_read < 0)
            return did_read;
        else if (did_read == 0)
            return read_total;
...
static sf_count_t mem_snd_seek(sf_count_t offset, int whence, void* datasource)
{
    file_reader_t* fr = (file_reader_t*)datasource;
    int64_t pos = 0;
    if (whence == SEEK_SET) pos = offset;
    if (whence == SEEK_CUR) pos += offset;
    if (whence == SEEK_END) pos = fr->len - offset;
    if (pos >= (int64_t)fr->len) pos = fr->len - 1;
    if (pos < 0) pos = 0;
    fr->pos = pos;
...
static sf_count_t mem_get_filelen(void* datasource)
{
    file_reader_t* fr = (file_reader_t*)datasource;
    return fr->len;
} /* mem_get_filelen */

static sound_stream_t* load_aiff(FILE* fp, long startpos, long length, int id,
                                 float volume, int repeats)
{
    ...
    SF_VIRTUAL_IO mem_cb = {.seek = mem_snd_seek,
                            .read = mem_snd_read,
    ...
    aiff->sndfile =
        sf_open_virtual(&mem_cb, SFM_READ, &aiff->sf_info, &aiff->freader);
...
static void sound_enqueue_real(sound_engine_t* e, sound_stream_t* s);
static void sound_stop_id_real(sound_engine_t* e, int id);

static void process_engine(sound_engine_t* e)
{
    int i;
    /* Handle event */
...
static void* audio_loop(void* v)
{
    ...
    while (1) {
        ...
        for (i = 0; i < (int)frotz_audio.buffer_size; ++i) {
            buf[2 * i + 0] =
                limit(-32764, 32767, mul * 0.8 * frotz_audio.outl[i]);
            buf[2 * i + 1] =
                limit(-32764, 32767, mul * 0.8 * frotz_audio.outr[i]);
        }
...
static void sound_halt_aiff(void)
{
    int i;
    for (i = 0; i < NUM_VOICES; ++i) {
        if (frotz_audio.streams[i] &&
            frotz_audio.streams[i]->sound_type == FORM) {
...
static void sound_halt_mod(void)
{
    int i;
    for (i = 0; i < NUM_VOICES; ++i) {
        if (frotz_audio.streams[i] &&
            frotz_audio.streams[i]->sound_type == MOD) {
...
static void sound_halt_ogg(void)
{
    int i;
    for (i = 0; i < NUM_VOICES; ++i) {
        if (frotz_audio.streams[i] &&
            frotz_audio.streams[i]->sound_type == OGGV) {
...
static void sound_stop_id(int id)
{
...
static void sound_enqueue(sound_stream_t* s)
{
    ux_sem_wait(&frotz_audio.ev_free);
...
static float volume_factor(int vol)
{
    static float lut[8] = {0.0078125f, 0.015625f, 0.03125f, 0.0625f,
...
void os_start_sample(int number, int volume, int repeats, zword eos)
{
    ...
    if (bb_err_None != bb_load_resource(blorb_map, bb_method_FilePos, &resource,
                                        bb_ID_Snd, number))
        return;
    ...
    if (type == FORM) {
        s = load_aiff(blorb_fp, resource.data.startpos, resource.length, number,
                      vol, repeats);
    } else if (type == MOD) {

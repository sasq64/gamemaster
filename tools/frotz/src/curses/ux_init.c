...
#    define HOMEDIR "HOME"
    ...
    if (signal(SIGINT, SIG_IGN) != SIG_IGN) signal(SIGINT, sigint_handler);

    if (signal(SIGTTIN, SIG_IGN) != SIG_IGN) signal(SIGTTIN, SIG_IGN);

    if (signal(SIGTTOU, SIG_IGN) != SIG_IGN) signal(SIGTTOU, SIG_IGN);
    ...
    if (p != NULL) *p = '\0'; /* extension removed */

    /* Create nice default file names */
    f_setup.script_name = malloc(
        (strlen(f_setup.story_name) + strlen(EXT_SCRIPT) + 1) * sizeof(char));
    memcpy(f_setup.script_name, f_setup.story_name,
           (strlen(f_setup.story_name) + strlen(EXT_SCRIPT)) * sizeof(char));
    strncat(f_setup.script_name, EXT_SCRIPT, strlen(EXT_SCRIPT) + 1);

    f_setup.command_name = malloc(
        (strlen(f_setup.story_name) + strlen(EXT_COMMAND) + 1) * sizeof(char));
    memcpy(f_setup.command_name, f_setup.story_name,
           (strlen(f_setup.story_name) + strlen(EXT_COMMAND)) * sizeof(char));
    strncat(f_setup.command_name, EXT_COMMAND, strlen(EXT_COMMAND) + 1);

    if (!f_setup.restore_mode) {
        f_setup.save_name = malloc(
            (strlen(f_setup.story_name) + strlen(EXT_SAVE) + 1) * sizeof(char));
        memcpy(f_setup.save_name, f_setup.story_name,
               (strlen(f_setup.story_name) + strlen(EXT_SAVE)) * sizeof(char));
        strncat(f_setup.save_name, EXT_SAVE, strlen(EXT_SAVE) + 1);
    } else { /*Set our auto load save as the name_save*/
        f_setup.save_name =
            malloc((strlen(f_setup.tmp_save_name) + strlen(EXT_SAVE) + 1) *
                   sizeof(char));
    ...
    f_setup.aux_name = malloc(
        (strlen(f_setup.story_name) + strlen(EXT_AUX) + 1) * sizeof(char));
...
static void parse_options(int argc, char** argv)
{
    ...
    do {
        c = zgetopt(argc, argv,
                    "aAb:c:deE:f:Fh:iI:l:L:moOpPqr:R:s:S:tu:vw:W:xZ:");
        switch (c) {
        case 'a':
        ...
        case 'b':
            ...
            break;
        case 'c':
            f_setup.context_lines = atoi(zoptarg);
            break;
        case 'd':
        ...
        case 'e':
            ...
            break;
        case 'E':
            u_setup.emphasis_mode = getemph(zoptarg);
            break;
        case 'f':
        ...
        case 'F':
            ...
            break;
        case 'h':
            u_setup.screen_height = atoi(zoptarg);
            break;
        case 'i':
        ...
        case 'I':
            ...
            break;
        case 'l':
            f_setup.left_margin = atoi(zoptarg);
            break;
        case 'L':
        ...
        case 'q':
            ...
            break;
        case 'r':
            f_setup.right_margin = atoi(zoptarg);
            break;
        case 'R':
            f_setup.restricted_path = strndup(zoptarg, PATH_MAX);
            break;
        case 's':
            u_setup.random_seed = atoi(zoptarg);
            break;
        case 'S':
        ...
        case 'v':
            ...
            break;
        case 'w':
            u_setup.screen_width = atoi(zoptarg);
            break;
        case 'x':
...
void os_init_screen(void)
{
    ...
    if (getenv("ESCDELAY") == NULL) setenv("ESCDELAY", "50", 1);
    ...
    if (z_header.version == V3) z_header.config |= CONFIG_SPLITSCREEN;
    ...
    if (z_header.version >= V5) z_header.flags &= ~(GRAPHICS_FLAG | MENU_FLAG);
    ...
    if (z_header.version >= V5 && (z_header.flags & UNDO_FLAG)) {
        if (f_setup.undo_slots == 0) z_header.flags &= ~UNDO_FLAG;
    }
    ...
    if (f_setup.interpreter_number == INTERP_DEFAULT)
        z_header.interpreter_number =
            z_header.version == V6 ? INTERP_MSDOS : INTERP_DEC_20;
    else
     ...
     */
    u_setup.color_enabled =
        (has_colors() && !u_setup.disable_color &&
     ...
     * understand.
     */
    if (u_setup.force_color) u_setup.color_enabled = TRUE;
...
FILE* os_path_open(const char* name, const char* mode)
{
    ...
    if (f_setup.zcode_path != NULL) {
        if ((fp = pathopen(name, f_setup.zcode_path, mode)) != NULL) return fp;
    }

    if ((p = getenv(PATH1)) == NULL) p = getenv(PATH2);
...
FILE* os_load_story(void)
{
    ...
    if (f_setup.exec_in_blorb) fseek(fp, blorb_res.data.startpos, SEEK_SET);
...
static int getconfig(char* configfile)
{
    ...
    while (fgets(varname, LINELEN, fp) != NULL) {
        ...
        for (num = strlen(varname) - 1;
             (ssize_t)num >= 0 && isspace((int)varname[num]); num--)
            ;
        varname[num + 1] = 0;

        /* Drop everything past the comment character */
        for (num = 0; num <= strlen(varname) + 1; num++) {
            if (varname[num] == COMMENT) varname[num] = 0;
        }

        /* Find end of variable name */
        for (num = 0;
             varname[num] != 0 && !isspace((int)varname[num]) && num < LINELEN;
             num++)
            ;
...
static int getemph(char* value)
{
    ...
    if (strncmp(value, "italic", 6) == 0) return EMPHASIS_ITALIC;

    if (strncmp(value, "under", 5) == 0) return EMPHASIS_UNDERLINE;

    if (strncmp(value, "none", 2) == 0) return EMPHASIS_NONE;

    if (atoi(value) == 0) return EMPHASIS_NONE;
...
static int getbool(char* value)
{
    ...
    if (strncmp(value, "y", 1) == 0) return TRUE;
...
static int getcolor(char* value)
{
    ...
    if (strcmp(value, "black") == 0) return BLACK_COLOUR;
    if (strcmp(value, "red") == 0) return RED_COLOUR;
    if (strcmp(value, "green") == 0) return GREEN_COLOUR;
    if (strcmp(value, "blue") == 0) return BLUE_COLOUR;
    if (strcmp(value, "magenta") == 0) return MAGENTA_COLOUR;
    if (strcmp(value, "cyan") == 0) return CYAN_COLOUR;
    if (strcmp(value, "white") == 0) return WHITE_COLOUR;
    if (strcmp(value, "yellow") == 0) return YELLOW_COLOUR;

    if (strcmp(value, "purple") == 0) return MAGENTA_COLOUR;
    if (strcmp(value, "violet") == 0) return MAGENTA_COLOUR;
    if (strcmp(value, "aqua") == 0) return CYAN_COLOUR;
...
static int geterrmode(char* value)
{
    ...
    if (strcmp(value, "never") == 0) return ERR_REPORT_NEVER;
    if (strcmp(value, "once") == 0) return ERR_REPORT_ONCE;
    if (strcmp(value, "always") == 0) return ERR_REPORT_ALWAYS;
    if (strcmp(value, "fatal") == 0) return ERR_REPORT_FATAL;
...
static void print_version(void)
{
    ...
    printf("  It is distributed under the GNU General Public License version 2 "
           "or\n");

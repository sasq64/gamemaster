...
void os_erase_area(int top, int left, int bottom, int right, int UNUSED(win))
{
    ...
    if ((top == 1) && (bottom == z_header.screen_rows) && (left == 1) &&
        (right == z_header.screen_cols)) {
#ifdef COLOR_SUPPORT
...
{
    int lines, cols;
    if (!saved_screen) return FALSE;
    if (xsize == 0 || ysize == 0) return TRUE;
    getmaxyx(saved_screen, lines, cols);
    ypos_old--, ypos_new--, xpos--;
    if (xpos + xsize > cols) xsize = cols - xpos;
    if (ypos_old + ysize > lines) ysize = lines - ypos_old;
    ...
    if (win == cwin) {
        ...
        y += ypos_new - ypos_old;
        if (y >= ypos_new && y < ypos_new + ysize && x >= xpos &&
            x < xpos + xsize)
            move(y, x);
    }

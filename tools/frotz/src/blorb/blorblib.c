...
static uint16 bb_native2(uint16 v)
{
    ...
    return ((((uint16)(v) >> 8) & 0x00ff) | (((uint16)(v) << 8) & 0xff00));
}
...
static uint32 bb_native4(uint32 v)
{
    if (bigendian) return v;
    return (
        (((uint32)(v) >> 24) & 0x000000ff) | (((uint32)(v) >> 8) & 0x0000ff00) |
        (((uint32)(v) << 8) & 0x00ff0000) | (((uint32)(v) << 24) & 0xff000000));
}
...
bb_err_t bb_create_map(FILE* file, bb_map_t** newmap)
{
    ...
    *newmap = NULL;
    ...
    err = fseek(file, 0, 0);
    if (err) return bb_err_Read;

    readlen = fread(buffer, sizeof(uint32), 3, file);
    if (readlen != 3) return bb_err_Read;

    if (bb_native4(buffer[0]) != bb_ID_FORM) return bb_err_Format;
    ...
    while (nextpos < totallength) {
        ...
        if (readlen != 2) return bb_err_Read;
        ...
        if (numchunks >= chunks_size) {
            chunks_size *= 2;
            chunks = (bb_chunkdesc_t*)realloc(chunks, sizeof(bb_chunkdesc_t) *
                                                          chunks_size);
        }
        ...
        if (nextpos > totallength) return bb_err_Format;
    }
...
static bb_err_t bb_initialize_map(bb_map_t* map)
{
    ...
    for (ix = 0; ix < map->numchunks; ix++) {
        ...
        case bb_ID_RIdx:
            ...
            if (gotindex) return bb_err_Format; /* duplicate index chunk */
            ...
            if (numres) {
                ...
                resources =
                    (bb_resdesc_t*)malloc(numres * sizeof(bb_resdesc_t));
                ressorted =
                    (bb_resdesc_t**)malloc(numres * sizeof(bb_resdesc_t*));
                if (!ressorted || !resources) return bb_err_Alloc;
                ...
                for (jx = 0; jx < numres; jx++) {
                    ...
                    if (ix2 >= map->numchunks ||
                        map->chunks[ix2].startpos != respos)
                        return bb_err_Format; /* start pos does not match a real
                                                 chunk */
                ...
                /* Sort a resource list (actually a list of pointers to
                   structures in map->resources.) This makes it easy to find
                   resources by usage and resource number. */
        ...
        case bb_ID_RelN:
            ...
            if (chunkres.length != 2) return bb_err_Format;
        ...
        case bb_ID_IFhd:
            ...
            if (chunkres.length < 13) return bb_err_Format;

            {
                uint16 val;
                bb_zheader_t* head =
                    (bb_zheader_t*)malloc(sizeof(bb_zheader_t));
                if (!head) return bb_err_Alloc;
        ...
        case bb_ID_Reso:
            ...
            if (chunkres.length < 24) return bb_err_Format;
...

            {
                bb_resolution_t* reso =
                    (bb_resolution_t*)malloc(sizeof(bb_resolution_t));
                if (!reso) return bb_err_Alloc;
            ...
            if (len) {
                bb_aux_pict_t* aux =
                    (bb_aux_pict_t*)malloc(len * sizeof(bb_aux_pict_t));
                ...
                for (jx = 0; (unsigned int)jx < len; jx++, ptr += 7) {
                    ...
                    if (!err) {
                        ...
                        if (chu->auxdatnum != -1)
                            return bb_err_Format; /* two image entries for this
                                                     resource */
                        chu->auxdatnum = jx;
        ...
        case bb_ID_Loop:
            ...
            if (len) {
                bb_aux_sound_t* aux =
                    (bb_aux_sound_t*)malloc(len * sizeof(bb_aux_sound_t));
                ...
                for (jx = 0; (unsigned int)jx < len; jx++, ptr += 2) {
                    ...
                    if (!err) {
                        bb_chunkdesc_t* chu = &(map->chunks[res.chunknum]);
                        if (chu->auxdatnum != -1)
                            return bb_err_Format; /* two looping entries for
                                                     this resource */
                        chu->auxdatnum = jx;
                                 ...
                                 int chunknum)
{
    ...
    if (chunknum < 0 || chunknum >= map->numchunks) return bb_err_NotFound;
                          ...
                          uint32 usage, int resnum)
{
    ...
    if (!found) return bb_err_NotFound;
...
bb_err_t bb_get_palette(bb_map_t* map, bb_palette_t** res)
{
    ...
    if (!map->palette) {
        ...
        if (chunkres.length == 1) {
            int val = ptr[0];
            if (val != 16 && val != 32) return bb_err_Format;
            pal->isdirect = TRUE;
            pal->data.depth = val;
        } else {
            int size = chunkres.length / 3;
            bb_color_t* colors = (bb_color_t*)malloc(size * sizeof(bb_color_t));
            if (!colors) return bb_err_Alloc;
            if (size < 1 || size > 256) return bb_err_Format;
            for (ix = 0; ix < size; ix++) {

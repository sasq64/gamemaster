...
#    define READ_DSP(v)                                                        \
        {                                                                      \
            while (!inp(sound_adr + 14) & 0x80)                                \
                ;                                                              \
            (v) = inportb(sound_adr + 10);                                     \
        }
#    define WRITE_DSP(v)                                                       \
        {                                                                      \
            while (inp(sound_adr + 12) & 0x80)                                 \
                ;                                                              \
            outportb(sound_adr + 12, (v));                                     \
        }
...
{
    static unsigned dma_page_port[] = {0x87, 0x83, 0x81, 0x82};
...
bool dos_init_sound(void)
{
    ...
    if ((settings = getenv("BLASTER")) == NULL) return FALSE;
...
void os_prepare_sample(int number)
{
    ...
    if (current_sample != number) {
        ...
        /* Open sample file */
        if ((fp = fopen(sample_name, "rb")) == NULL) return;

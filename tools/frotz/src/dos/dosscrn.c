...
static void clear_byte(volatile byte _far* scrn, word mask)
{
    if (display == _CGA_)
        ...
        *scrn = scrn_attr;
        ...
        asm les bx, scrn asm mov al, es
            : [bx] asm mov al, scrn_attr asm mov es
            : [bx], al
#endif
    }
} /* clear_byte */
...
static void clear_line(int y, int left, int right)
{
    ...
    if (display == _MCGA_)
...
#ifdef __WATCOMC__
        _fmemset((byte _far*)scrn + left, scrn_attr, right - left + 1);
#else
        ...
        /* Clear middle bytes */
        if (display >= _EGA_) outport(0x03ce, 0xff08);
        while (--x > 0)
            *scrn++ = scrn_attr;
...
void os_erase_area(int top, int left, int bottom, int right, int win)
{
    ...
    if (display <= _TEXT_) {
#ifdef __WATCOMC__
        bios_video_ax_bh_ch_cl_dh_dl(0x0600, scrn_attr, top, left, bottom,
                                     right);
#else
        asm mov ax, 0x0600 asm mov ch, byte ptr top asm mov cl,
            byte ptr left asm mov dh, byte ptr bottom asm mov dl,
            byte ptr right asm mov bh, scrn_attr asm int 0x10
#endif
    } else
...
#else
            asm les bx, scrn2 asm mov ah, es
                : [bx] asm les bx, scrn1 asm mov al, es
                : [bx] asm mov es
                : [bx], ah
        }
#endif
...
static void copy_line(int y1, int y2, int left, int right)
{
    ...
    if (display == _MCGA_)
#ifdef __WATCOMC__
        _fmemcpy((byte _far*)(scrn1 + left), (byte _far*)(scrn2 + left),
                 right - left + 1);
#else
        ...
        /* Copy middle bytes */
        if (display >= _EGA_) outport(0x03ce, 0x0105);
        while (--x > 0)
            *scrn1++ = *scrn2++;
...
void os_scroll_area(int top, int left, int bottom, int right, int units)
{
    ...
    if (display <= _TEXT_) {
        ...
        bios_video_ax_bh_ch_cl_dh_dl(ax, scrn_attr, top, left, bottom, right);
#else
        asm mov ah, 6 asm mov bx, units asm cmp bx, 0 asm jg scroll asm mov ah,
            7 asm neg bx scroll
            : asm mov al, bl asm mov ch, byte ptr top asm mov cl,
              byte ptr left asm mov dh, byte ptr bottom asm mov dl,
              byte ptr right asm mov bh, scrn_attr asm int 0x10
#endif
    } else if (units > 0) {
...
bool os_repaint_window(int win, int ypos_old, int ypos_new, int xpos, int ysize,
                       int xsize)
{
    return FALSE;
} /* os_repaint_window */

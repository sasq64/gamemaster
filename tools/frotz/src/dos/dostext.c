...
char latin1_to_ascii[] = "   !  c  L  >o<Y  |  S  '' C  a  << not-  R  _  "
...
char latin1_to_ibm[] = {
    0x20, 0xad, 0xbd, 0x9c, 0xcf, 0xbe, 0xdd, 0xf5, 0xf9, 0xb8, 0xa6, 0xae,
...
int os_font_data(int font, int* height, int* width)
{
    ...
    *width = z_header.font_width;
    ...
    if (font == TEXT_FONT) return TRUE;
    if (font == GRAPHICS_FONT && (display == _CGA_ || display >= _EGA_))
        return TRUE;
    if (font == FIXED_WIDTH_FONT) return TRUE;
...
static void adjust_style(void)
{
    static byte amiga_palette[][3] = {
        {0x00, 0x00, 0x00}, {0x2a, 0x00, 0x00}, {0x00, 0x2a, 0x00},
        {0x3f, 0x3f, 0x15}, {0x00, 0x00, 0x2a}, {0x2a, 0x00, 0x2a},
        {0x00, 0x2a, 0x2a}, {0x3f, 0x3f, 0x3f}, {0x30, 0x30, 0x30},
        {0x20, 0x20, 0x20}, {0x10, 0x10, 0x10},
    };

    static byte pc_colour[] = {BLACK,   RED,  GREEN, YELLOW,  BLUE,
...
#else
            asm mov ax, 0x1010 asm mov bx, 79 asm mov dh, R asm mov ch,
                G asm mov cl,
                B asm int 0x10
#endif
            palette_fg = fg;
...
#else
            asm mov ax, 0x1010 asm mov bx, 64 asm mov dh, R asm mov ch,
                G asm mov cl,
                B asm int 0x10
#endif
            palette_bg = bg;
    ...
    if (current_style & BOLDFACE_STYLE) {
        if (display == _MONO_) text_fg = WHITE;
        if (display == _TEXT_) text_fg ^= 8;
    }
...
void write_pattern(volatile byte _far* screen, byte val, byte mask)
{
    ...
    if (mask != 0) {
        if (display == _CGA_) {
            if (bg == BLACK) *screen &= ~mask;
            if (bg == WHITE) *screen |= mask;
            if (fg != bg) *screen ^= val;
            ...
            *screen = fg;
#else
            asm mov dx, 0x03cf asm mov al, mask asm out dx, al asm les bx,
                screen asm mov ch, text_bg asm mov al, es
                : [bx] asm mov es
                : [bx], ch asm mov al, val asm out dx, al asm mov ch,
                  text_fg asm mov al, es
                : [bx] asm mov es
                : [bx], ch
#endif
        }
...
void os_display_char(zchar c)
{
    ...
    if (c >= ZC_LATIN1_MIN &&
        ...
        if (display == _CGA_ || display == _MCGA_) {
            ...
            if (c2 != ' ') os_display_char(c2);
            if (c3 != ' ') os_display_char(c3);

            return;

        } else if (display == _AMIGA_ && current_font == TEXT_FONT &&
                   !(current_style & FIXED_WIDTH_STYLE) && user_font != 0) {

            if (c >= ZC_LATIN1_MIN) c -= 32;
        } else
    ...
    if (display <= _TEXT_) {
        ...
        asm mov ah, 2 asm mov bh, 0 asm mov dh, byte ptr cursor_y asm mov dl,
            byte ptr cursor_x asm int 0x10 asm mov ah, 9 asm mov bh,
            0 asm mov bl, byte ptr text_bg asm mov cl, 4 asm shl bl,
            cl asm or bl, byte ptr text_fg asm mov cx, 1 asm mov al,
            byte ptr c asm int 0x10
#endif
    } else {
        void _far* table;
        word mask;
        ...
        if (!(current_style & BOLDFACE_STYLE)) boldface = -1;
        if (!(current_style & EMPHASIS_STYLE)) underline = -1;
        ...
        for (i = 0; i < z_header.font_height; i++) {
            volatile byte _far* screen = get_scrnptr(cursor_y + i) + offset;
            if (type == 1) {
                val = *((byte _far*)table + 8 * i / z_header.font_height);
            }
            if (type == 2) val = *((word _far*)table + i);
            if (type == 3) val = *((byte _far*)table + i);

            if (align != 0) val >>= align;

            if (boldface == 1) val |= val >> 1;
            if (underline == i) val ^= mask;
...
int os_char_width(zchar c)
{
    ...
    if (c >= ZC_LATIN1_MIN &&
        ...
        if (display == _CGA_ || display == _MCGA_) {
            ...
            if (c2 != ' ') width += os_char_width(c2);
            if (c3 != ' ') width += os_char_width(c3);
    ...
    /* Handle special indentations */
    if (c == ZC_INDENT) return 3 * os_char_width(' ');
    if (c == ZC_GAP) return 2 * os_char_width(' ');

    /* Calculate width */
    if (display <= _TEXT_) return 1;
    if (display == _CGA_) return 8;
    if (display == _EGA_) return 8;

    if (current_font == GRAPHICS_FONT) return 8;
    if (current_font == FIXED_WIDTH_FONT ||
        (current_style & FIXED_WIDTH_STYLE) ||
        (display == _AMIGA_ && user_font == 0))
        return (display == _AMIGA_) ? 8 : 5;

    if (display == _MCGA_) return mcga_width[c - 32];
    if (display == _AMIGA_)
        return sans_width[c - 32] + ((current_style & BOLDFACE_STYLE) ? 1 : 0);
...
int os_string_width(const zchar* s)
{
    ...
    while ((c = *s++) != 0) {
        if (c == ZC_NEW_STYLE || c == ZC_NEW_FONT) {
            ...
            if (c == ZC_NEW_FONT) current_font = arg;
            if (c == ZC_NEW_STYLE) current_style = arg;
...
void os_more_prompt(void)
{
    ...
    os_erase_area(cursor_y + 1, saved_x + 1, cursor_y + z_header.font_height,
                  cursor_x + 1, -1);

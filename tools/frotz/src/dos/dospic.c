...
#    endif
    ...
    if ((file = fopen(fname, "rb")) == NULL) goto failure1;
    if (fread(&gheader, sizeof(gheader), 1, file) != 1) goto failure2;
#    ifdef __WATCOMC__
    images = gheader.images;
    entry_size = gheader.entry_size;
    if ((info = halloc((long)images * entry_size, 1)) == NULL) goto failure2;
    p = info;
    while (images != 0) {
...
bool init_pictures(void)
{
    ...
    if (table_val == NULL) return FALSE;
...
#    else
    asm mov ax, 0x1012 asm mov bx, first_colour asm mov cx, n asm lea dx,
        rgb asm push ss asm pop es asm int 0x10
#    endif
} /* load_colour_map */
...
#    endif
{
    static int raise_bits[4] = {0x0100, 0x0300, 0x0700, 0x0000};
...
#    endif
    ...
    /* When the given picture provides a colour map then activate it.
       ...
       Amiga mode.) */
    if (display == _CGA_) colour_shift = -2;
    if (display == _EGA_) colour_shift = 0;
    if (display == _MCGA_) {
        colour_shift = 32;
        first_colour = 34;
    }
    ...
    if (pic_colour != 0) load_colour_map(first_colour);
    ...
    if (pic_flags & 1) transparent = pic_flags >> 12;
...
next_code:
    ...
    if (next_entry == prev_code) buf[0] = code;
    ...
    if (next_entry == raise_bits[bits_per_code - 9]) bits_per_code++;
...
#    else
        asm les bx, screen asm mov dx,
            current_x asm dec dx asm push dx asm mov cl, 3 asm shr dx,
            cl asm add bx, dx asm mov ax, es
            : [bx] asm mov dx, 0xffff asm mov cl, byte ptr pixels asm shr dl,
              cl asm pop cx asm and cl, 7 asm ror dx, cl asm and ax,
              dx asm mov dx, code asm inc dh asm ror dx, cl asm or ax,
              dx asm mov es
            : [bx], ax
#    endif
        current_x += pixels;
...
#    else
            _AH = code;
            if (_AH != transparent) {
                asm add ah, colour_shift asm les bx, screen asm mov dx,
                    current_x asm dec dx if (display != _MCGA_)
                {
                    asm push dx asm mov cl, 3 asm shr dx,
                        cl asm pop cx asm and cl, 7 asm add bx, dx asm mov al,
                        es
                        : [bx] if (display == _CGA_)
                    {
                        asm mov dl, 0x7f asm ror dl, cl asm and al,
                            dl asm xor ah, 1 asm ror ah, 1 asm shr ah,
                            cl asm or ah, al
                    }
                    else
                    {
                        asm mov al, 0x80 asm shr al, cl asm mov dx,
                            0x03cf asm out dx, al
                    }
                }
                else asm add bx,
                    dx
                    ...
                    asm mov es : [bx],
                                 ...
                                 if (display == _AMIGA_)
                {
                    asm add bx, 80 asm mov al, es : [bx] asm mov es : [bx], ah
                }
#    endif
            }
            current_x++;
...
{
    if (load_picture_info(picture)) draw_picture(y, x);
} /* os_draw_picture */
...
int os_peek_colour(void)
{
    if (display >= _CGA_) {
#    ifdef __WATCOMC__
        return (int)(byte)bios_video_ah_bh_cx_dx(13, 0, cursor_x, cursor_y) +
               16;
#    else
        asm mov ah, 13 asm mov bh, 0 asm mov cx, cursor_x asm mov dx,
            cursor_y asm int 0x10 asm mov ah, 0 return _AX + 16;
#    endif
    }
    return current_bg;
} /* os_peek_colour */

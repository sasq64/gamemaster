...
#else
    asm mov ah, 0 asm mov al,
        old_video_mode asm int 0x10 setvect(0x1b, oldvect);
#endif
} /* cleanup */
...
{
    if (z_header.interpreter_number) os_reset_screen();
...
static void parse_options(int argc, char** argv)
{
    ...
    do {
        ...
        if (zoptarg != NULL) num = dectoi(zoptarg);

        if (c == 'a') f_setup.attribute_assignment = 1;
        if (c == 'A') f_setup.attribute_testing = 1;
        if (c == 'b') user_background = num;
        if (c == 'B') user_reverse_bg = num;
        if (c == 'c') f_setup.context_lines = num;
        if (c == 'd') {
        ...
        if (c == 'e') user_emphasis = num;
        if (c == 'T') user_bold_typing = 1;
        if (c == 'f') user_foreground = num;
        if (c == 'F') user_reverse_fg = num;
        if (c == 'g') {
            if (num >= 0 && num <= 4) user_font = num;
        }
        if (c == 'h') user_screen_height = num;
        if (c == 'i') f_setup.ignore_errors = 1;
        if (c == 'l') f_setup.left_margin = num;
        if (c == 'o') f_setup.object_movement = 1;
        if (c == 'O') f_setup.object_locating = 1;
        if (c == 'p') f_setup.piracy = 1;
        if (c == 'q') f_setup.quiet = 1;
        if (c == 'r') f_setup.right_margin = num;
        if (c == 'R') f_setup.restricted_path = strdup(zoptarg);
        if (c == 's') user_random_seed = num;
        if (c == 'S') f_setup.script_cols = num;
        if (c == 't') f_setup.tandy = 1;
        if (c == 'u') f_setup.undo_slots = num;
        if (c == 'v') {
            ...
            printf(
                "  It complies with standard 1.1 of the Z-Machine Standard.\n");
            printf("  It was ported to Unix by Galen Hazelwood.\n");
            printf(
                "  The core and DOS port are maintained by David Griffith.\n");
            printf("  Frotz's homepage is https://661.org/proj/if/frotz/\n");
            exit(0);
        }
        if (c == 'w') user_screen_width = num;
        if (c == 'x') f_setup.expand_abbreviations = 1;
        if (c == 'Z') {
...
static void standard_palette(void)
{
    ...
    static byte palette[] = {
        0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x14, 0x07, 0x38,
        0x39, 0x3a, 0x3b, 0x3c, 0x3d, 0x3e, 0x3f, 0x00 /* last one is the
                                                          overscan register */
    };
...
#else
    if (display == _AMIGA_) {
        asm mov ax, 0x1002 asm lea dx,
            palette asm push ds asm pop es asm int 0x10 asm mov ax,
            0x1013 asm mov bx, 0x0001 asm int 0x10
    }
#endif
} /* standard_palette */
...
static void special_palette(void)
{
    ...
    static byte palette[] = {
        ...
        0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x00 /* last one is the
                                                          overscan register */
    };
...
#else
    if (display == _AMIGA_) {
        asm mov ax, 0x1002 asm mov dx,
            offset palette asm push ds asm pop es asm int 0x10 asm mov ax,
            0x1013 asm mov bx, 0x0101 asm int 0x10
    }
#endif
} /* special_palette */
...
{
    static byte zcolour[] = {
        BLACK_COLOUR,  BLUE_COLOUR,       GREEN_COLOUR,    CYAN_COLOUR,
        RED_COLOUR,    MAGENTA_COLOUR,    BROWN + 16,      LIGHTGRAY + 16,
        GREY_COLOUR,   LIGHTBLUE + 16,    LIGHTGREEN + 16, LIGHTCYAN + 16,
        LIGHTRED + 16, LIGHTMAGENTA + 16, YELLOW_COLOUR,   WHITE_COLOUR};

    static struct
    { /* information on modes 0 to 5 */
        byte vmode;
        word width;
        word height;
        byte font_width;
...
#else
    asm mov ah, 15 asm int 0x10 asm mov old_video_mode,
        al
#endif
...
#else
        /* Enable bright background colours */
        asm mov ax, 0x1003 asm mov bl,
            0 asm int 0x10
            /* Turn off hardware cursor */
            asm mov ah,
            1 asm mov cx, 0xffff asm int 0x10
#endif
    } else {
        if (display == _AMIGA_) {
            scaler = 2;
    ...
    if (z_header.flags & GRAPHICS_FLAG)
        if (display <= _TEXT_) z_header.flags &= ~GRAPHICS_FLAG;
    ...
    if (z_header.version >= V5 && (z_header.flags & UNDO_FLAG))
        if (!f_setup.undo_slots) z_header.flags &= ~UNDO_FLAG;
    if (z_header.flags & MOUSE_FLAG)
        if (subdisplay != -1 || !detect_mouse()) z_header.flags &= ~MOUSE_FLAG;
    ...
    if (user_screen_width != -1) z_header.screen_width = user_screen_width;
    if (user_screen_height != -1) z_header.screen_height = user_screen_height;
    ...
    if (display == _AMIGA_) z_header.interpreter_number = INTERP_AMIGA;
    ...
    if (story_id == BEYOND_ZORK) {
        if (stage == RESTART_BEGIN) {
            if ((display == _MCGA_ || display == _AMIGA_) &&
                ...
                bios_video_ax_bx_dh_ch_cl(0x1010, 79, 0xff, 0xff, 0xff);
#else asm mov ax, 0x1010
                asm mov bx, 64 asm mov dh, 0 asm mov ch, 0 asm mov cl,
                    0 asm int 0x10 asm mov ax, 0x1010 asm mov bx, 79 asm mov dh,
                    0xff asm mov ch, 0xff asm mov cl,
                    0xff asm int 0x10
#endif
...
#else
        asm mov ah, 0 asm int 0x1a return _DX & 0x7fff;
#endif
    } else
...
FILE* os_path_open(const char* name, const char* mode)
{
    ...
    if ((p = getenv("ZCODE_PATH")) == NULL) p = getenv("INFOCOM_PATH");
    if (p != NULL) {
        while (*p) {
            bp = buf;
            while (*p && *p != OS_SEARCH_PATHSEP)
                lastch = *bp++ = *p++;
            if (lastch != '\\' && lastch != '/') *bp++ = '\\';
            strcpy(bp, name);
            if ((fp = fopen(buf, mode)) != NULL) return fp;
            if (*p) p++;
        }
    }
    return NULL;
...
#ifndef NO_BLORB
    /* Is this a Blorb file containing Zcode? */
    if (f_setup.exec_in_blorb) return ftell(fp) - blorb_res.data.startpos;
#endif
    return ftell(fp);
} /* os_storyfile_tell */

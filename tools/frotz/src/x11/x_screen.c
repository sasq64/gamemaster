...
void os_scroll_area(int top, int left, int bottom, int right, int units)
{
    if (units == 0)
        ...
        XCopyArea(dpy, main_window, main_window, current_gc, left - 1,
                  top - 1 + units, right - left + 1, bottom - top - units + 1,
                  left - 1, top - 1);
        XFillRectangle(dpy, main_window, reversed_gc, left - 1, bottom - units,
                       right - left + 1, units);
    } else {
        XCopyArea(dpy, main_window, main_window, current_gc, left - 1, top - 1,
                  right - left + 1, bottom - top + units + 1, left - 1,
                  top - 1 + units);
        XFillRectangle(dpy, main_window, reversed_gc, left - 1, top - 1,
                       right - left + 1, -units);
    }

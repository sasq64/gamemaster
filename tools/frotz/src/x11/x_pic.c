...
static void lookup_color(XColor* x)
{
    ...
    if (is_truecolor) {
        ...
        if (b) b -= 1;
        x->pixel =
            colortab[((r / 0xffff) * red_mult) + ((g / 0xffff) * green_mult) +
                     ((b / 0xffff) * blue_mult)];
    }
}
...
bool x_init_pictures(void)
{
    ...
    if (reso) {
        if (reso->px) blrb_width = reso->px;
        if (reso->py) blrb_height = reso->py;
    }
    bb_count_resources(blorb_map, bb_ID_Pict, &num_pictures, NULL,
                       &maxlegalpic);
    ...
    if (bb_load_chunk_by_type(blorb_map, bb_method_Memory, &res, bb_ID_APal,
                              0) == bb_err_None) {
        m_adaptiveMode = TRUE;
    ...
    for (i = 0, j = 0; (i < num_pictures) && (j <= maxlegalpic); j++) {
        if (bb_load_resource(blorb_map, bb_method_Memory, &res, bb_ID_Pict,
                             j) == bb_err_None) {
            i++;
...
{
    int i;
    if (pict_info == NULL) return -1;
    for (i = 0; i <= num_pictures; i++) {
        if (pict_info[i].z_num == n) return i;
    }
    return -1;
} /* z_num_to_index */
...
{
    uint32_t r;
    r = fromLinear[(toLinear[s & 0xff] * a + toLinear[d & 0xff] * (256 - a)) >>
                   8];
    s >>= 8;
    d >>= 8;
    r |=
        (fromLinear[(toLinear[s & 0xff] * a + toLinear[d & 0xff] * (256 - a)) >>
                    8])
        << 8;
    s >>= 8;
    d >>= 8;
...
static int loadpng(uint8_t* data, int UNUSED(length), pict_info_t* graphic)
{
    ...
    if (!png_check_sig(data, 8)) return 0;
    ...
    if (m_adaptiveMode && (color_type == PNG_COLOR_TYPE_PALETTE) &&
        bit_depth <= 4) {
        ...
        if (png_get_PLTE(png_ptr, info_ptr, &palette, &num_palette)) {
            ...
            for (int i = 0; i < num_palette; i++) {
                uint32_t color = palette[i].red | (palette[i].green << 8) |
                                 (palette[i].blue << 16);
...
static int loadjpeg(uint8_t* data, int length, pict_info_t* graphic)
{
    ...
    buffer = (*info.mem->alloc_sarray)((j_common_ptr)&info, JPOOL_IMAGE,
                                       width * 3, 1);
    ...
    while ((int)info.output_scanline < height) {
        ...
        pixelRow = graphic->pixels + (width * (info.output_scanline - 1) * 4);
        for (i = 0; i < width; i++) {
...
bool x_IsAdaptive(int picture)
{
    ...
    if (bb_load_chunk_by_type(blorb_map, bb_method_Memory, &result, bb_ID_APal,
                              0) == bb_err_None) {
...
static int x_loadpic(int picture, pict_info_t* graphic)
{
    ...
    if (bb_load_resource(blorb_map, bb_method_Memory, (bb_result_t*)&res,
                         bb_ID_Pict, picture) == bb_err_None) {
...
pict_info_t* x_getpic(int num)
{
    ...
    if ((index = z_num_to_index(num)) == -1) return NULL;

    res = &pict_info[z_num_to_index(num)];

    if (res->pixels) return res;
    if (x_loadpic(num, res)) return res;
    return NULL;
}
...
void os_draw_picture(int picture, int y, int x)
{
    ...
    mask_image = NULL;
    if ((data = x_getpic(picture)) == NULL) return;
    if (data->pixels == NULL) return;

    contents_image =
        XCreateImage(dpy, DefaultVisual(dpy, DefaultScreen(dpy)),
                     DefaultDepth(dpy, DefaultScreen(dpy)), ZPixmap, 0, NULL,
                     data->width, data->height, 32, 0);
    ...
    if (data->transparentcolor != -1) {
        mask_image =
            XCreateImage(dpy, DefaultVisual(dpy, DefaultScreen(dpy)),
                         ...
                         XYBitmap, 0, NULL, data->width, data->height, 8, 0);
    ...
    for (ypos = 0; ypos < data->orig_height; ypos++)
        for (xpos = 0; xpos < data->orig_width; xpos++) {
            if (data->usespalette) {
                ...
                if (!data->adaptive) ApplyPalette(data);
                pixel_value.blue =
                    (screen_palette[image_value] & 0xff0000) >> 8;
                pixel_value.green = (screen_palette[image_value] & 0x00ff00);
                pixel_value.red = (screen_palette[image_value] & 0x0000ff) << 8;
            } else {
                pixel_value.blue =
                    data->pixels[(ypos * data->orig_width + xpos) * 4 + 2] *
                    256;
                pixel_value.green =
                    data->pixels[(ypos * data->orig_width + xpos) * 4 + 1] *
                    256;
                pixel_value.red =
                    data->pixels[(ypos * data->orig_width + xpos) * 4] * 256;
                image_value = (pixel_value.blue << 8) + (pixel_value.green) +
                              (pixel_value.red >> 8);
            }
            ...
            for (im_y = ypos * X_HEIGHT / blrb_height;
                 im_y < (ypos + 1) * X_HEIGHT / blrb_height; im_y++)
                for (im_x = xpos * X_WIDTH / blrb_width;
                     im_x < (xpos + 1) * X_WIDTH / blrb_width; im_x++)
                    XPutPixel(contents_image, im_x, im_y, pixel_value.pixel);
    ...
    if (mask_image) {
        ...
        XPutImage(dpy, main_window, bw_gc, mask_image, 0, 0, x - 1, y - 1,
                  data->width, data->height);
    }
    XSetFunction(dpy, bw_gc, ((data->transparentcolor != -1) ? GXor : GXcopy));
    XPutImage(dpy, main_window, bw_gc, contents_image, 0, 0, x - 1, y - 1,
              data->width, data->height);
...
int os_peek_colour(void)
{
    ...
    point =
        XGetImage(dpy, main_window, curr_x, curr_y, 1, 1, AllPlanes, ZPixmap);
...
bool os_picture_data(int picture, int* height, int* width)
{
    ...
    *width = 0;
    ...
    if (pict_info == NULL || !pict_info) return FALSE;
    if ((index = z_num_to_index(picture)) == -1) return FALSE;

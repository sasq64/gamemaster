...
void os_fatal(const char* s, ...)
{
    ...
    if (x_setup.x_active) os_reset_screen();
...
void os_process_arguments(int argc, char* argv[])
{
    ...
    static struct
    {
        ...
        {".ContextLines", ".contextLines", parse_int, &f_setup.context_lines},
        ...
        {".Margin", ".leftMargin", parse_int, &f_setup.left_margin},
        {".Margin", ".rightMargin", parse_int, &f_setup.right_margin},
        {".IgnoreErrors", ".ignoreErrors", parse_boolean,
         &f_setup.ignore_errors},
        {".Piracy", ".piracy", parse_boolean, &f_setup.piracy},
        {".Restricted", ".restricted", parse_string, &f_setup.restricted_path},
        {".Tandy", ".tandy", parse_boolean, &f_setup.tandy},
        {".UndoSlots", ".undoSlots", parse_int, &f_setup.undo_slots},
        {".ShowVersion", ".showVersion", parse_boolean, &show_version},
        {".ExpandAbbrevs", ".expandAbbrevs", parse_boolean,
         &f_setup.expand_abbreviations},
        {".ScriptColumns", ".scriptColumns", parse_int, &f_setup.script_cols},
        {".RandomSeed", ".randomSeed", parse_int, &x_setup.random_seed},
        {".ZStrict", ".zStrict", parse_zstrict, &f_setup.err_report_mode},
        {".Background", ".background", parse_string, &user_bg},
        {".Foreground", ".foreground", parse_string, &user_fg},
        {".Font", ".font", parse_string, &font_names[0]},
        {".Font", ".fontB", parse_string, &font_names[1]},
        {".Font", ".fontI", parse_string, &font_names[2]},
        {".Font", ".fontBI", parse_string, &font_names[3]},
        {".Font", ".fontF", parse_string, &font_names[4]},
        {".Font", ".fontFB", parse_string, &font_names[5]},
        {".Font", ".fontFI", parse_string, &font_names[6]},
        {".Font", ".fontFBI", parse_string, &font_names[7]},
        {".Font", ".fontZ", parse_string, &font_names[8]},
        {".LoadThis", ".loadthis", parse_string, &f_setup.tmp_save_name}};
    ...
    dpy = XtOpenDisplay(app_context, NULL, NULL, "XFrotz", options,
                        XtNumber(options), &argc, argv);
    if (dpy == NULL) os_fatal("Could not open display.");
    ...
    if (show_version) {
        ...
        printf("  It is distributed under the GNU General Public License "
               "version 2 or\n");
        printf("    (at your option) any later version.\n");
        printf("  This software is offered as-is with no warranty or "
               "liability.\n");
    ...
    if (f_setup.tmp_save_name != NULL) f_setup.restore_mode = 1;

    f_setup.story_file = strdup(argv[1]);
    f_setup.story_name = strdup(basename(argv[1]));

    if (argc > 2) f_setup.blorb_file = strdup(argv[2]);
    ...
    f_setup.script_name = malloc(
        (strlen(f_setup.story_name) + strlen(EXT_SCRIPT)) * sizeof(char) + 1);
    ...
    f_setup.command_name = malloc(
        (strlen(f_setup.story_name) + strlen(EXT_COMMAND)) * sizeof(char) + 1);
    ...
    if (!f_setup.restore_mode) {
        f_setup.save_name = malloc(
            (strlen(f_setup.story_name) + strlen(EXT_SAVE)) * sizeof(char) + 1);
        ...
        f_setup.save_name = malloc(
            (strlen(f_setup.tmp_save_name) + strlen(EXT_SAVE)) * sizeof(char) +
            1);
        memcpy(f_setup.save_name, f_setup.tmp_save_name,
               (strlen(f_setup.tmp_save_name) + strlen(EXT_SAVE)) *
                   sizeof(char));
        /* No need to free f_setup.tmp_save_name */
    }

    f_setup.aux_name = malloc(
        (strlen(f_setup.story_name) + strlen(EXT_AUX)) * sizeof(char) + 1);
...
void os_init_screen(void)
{
    ...
    main_window =
        XCreateWindow(dpy, DefaultRootWindow(dpy), 0, 0, X_WIDTH, X_HEIGHT, 0,
    ...
    if (!current_font_info) os_fatal("Could not open default font");
    ...
    cursor_gc = XCreateGC(
        dpy, main_window,
        GCFunction | GCForeground | GCBackground | GCFillStyle, &gc_setup);

    /* Let os_fatal know the graphics contexts have been initialized */
    x_setup.x_active = 1;
    ...
    bgpm = XCreatePixmap(dpy, main_window, X_WIDTH, X_HEIGHT,
...
{
    memset(&x_setup, 0, sizeof(x_setup));
    return;
}

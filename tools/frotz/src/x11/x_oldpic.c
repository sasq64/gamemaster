...
static struct
{
    int fileno, flags, unused1, images, link, entry_size, padding, checksum,
        unused2, version;
} gheader;
...
static struct picture_data* find_picture(int picture)
{
    ...
    for (i = 0; i < gheader.images; i++)
        if (gtable[i].number == picture) return gtable + i;
    return NULL;
}
...
static int decompress(void)
{
    ...
    if (bufpos == 0) {
        ...
        if (current_code < 4096) prev_seq[current_code - 256] = prev_code;
        for (val = code; val > 256; val = prev_seq[val - 256])
            buf[bufpos++] = last[val - 256];
        buf[bufpos++] = val;
        if (code == current_code) buf[0] = val;

        if (current_code < 4096) last[current_code - 256] = val;
        prev_code = code;
...
void os_draw_picture(int picture, int y, int x)
{
    ...
    if (graphics_fp == NULL)
        if (!open_graphics_file()) return;

    if ((data = find_picture(picture)) == NULL) return;
    ...
    if (data->flags & 1) {
        ...
        mask_image = XCreateImage(
            dpy, DefaultVisual(dpy, DefaultScreen(dpy)),
            /* DefaultDepth(dpy, DefaultScreen(dpy)), */ 1, XYBitmap, 0, NULL,
    ...
    for (ypos = 0; ypos < data->height; ypos++)
        for (xpos = 0; xpos < data->width; xpos++) {
            ...
            if (image_value == transparent) pixel_value = 0;
    ...
    if (data->flags & 1) XDestroyImage(mask_image);
} /* os_draw_picture */
...
int os_picture_data(int picture, int* height, int* width)
{
    ...
    if (graphics_fp == NULL)
        if (!open_graphics_file()) return 0;

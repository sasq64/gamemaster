 ...
 *
 */
zchar os_read_line(int max, zchar* buf, int timeout, int UNUSED(width),
                   int UNUSED(continued))
{
    zchar c;
    int index;
 ...
 *
 */
zchar os_read_key(int UNUSED(timeout), bool cursor)
{
    XEvent ev;
    char result[2];
    int size;
    ...
    if (cursor)
        XFillRectangle(dpy, main_window, cursor_gc, curr_x, curr_y, 2,
                       text_height);
    XCopyArea(dpy, main_window, bgpm, current_gc, 0, 0, X_WIDTH, X_HEIGHT, 0,
              0);
    for (;;) {
        XNextEvent(dpy, &ev);
        if (ev.type == KeyPress) {
            ...
            if (cursor)
                XFillRectangle(dpy, main_window, cursor_gc, curr_x, curr_y, 2,
                               text_height);
            size = XLookupString(key_ev, result, sizeof result, &symbol, NULL);
            if (size == 1) {
                if (key_ev->state & Mod1Mask) switch (result[0]) {
                    case 'r':
...
char* os_read_file_name(const char* default_name, int flag)
{
    ...
    zchar read_name[FILENAME_MAX + 1];
    int i;
    char* tempname;
    zchar answer[4];
    ...
    if (f_setup.restore_mode || flag == FILE_NO_PROMPT) {
        ...
        read_string(FILENAME_MAX - EXT_LENGTH, read_name);
        for (i = 0; i <= FILENAME_MAX - EXT_LENGTH; i++) {
            file_name[i] = translate_from_zscii(read_name[i]);

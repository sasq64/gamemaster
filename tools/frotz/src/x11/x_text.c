...
static int x_alloc_colour(int index, unsigned long* result)
{
    ...
    static char* fallback_names[] = {"black",   "red3",   "green3",
                                     "yellow3", "blue3",  "magenta3",
                                     "cyan3",   "gray80", "gray50"};
    XColor screen, exact;
    int retval;
    XrmValue value;
    ...
    if (XrmGetResource(XtDatabase(dpy), name_buf, class_buf, &str_type_return,
                       &value))
                         ...
                         int def_index)
{
    ...
    if (XAllocNamedColor(dpy, DefaultColormap(dpy, DefaultScreen(dpy)),
        ...
        for (z_num = BLACK_COLOUR; z_num <= GREY_COLOUR; z_num++)
            if (pixel_values[z_num] == pixel_values[index]) return z_num;
        return index;
    } else {
...
void x_init_colour(char* bg_name, char* fg_name)
{
    ...
    z_header.default_background = x_bg_fg_color(&def_bg_pixel, 14, bg_name, 9);
    z_header.default_foreground = x_bg_fg_color(&def_fg_pixel, 15, fg_name, 2);
}

char* font_names[9] = {"-adobe-times-medium-r-normal--14-*-*-*-*-*-iso8859-1",
...
const XFontStruct* get_font(int font, int style)
{
    ...
    if (font == FIXED_WIDTH_FONT) style |= (FIXED_WIDTH_STYLE >> 1);
    if (font == GRAPHICS_FONT) style = 8;
...
void os_display_char(zchar c)
{
    ...
    XDrawImageString(dpy, main_window, current_gc, curr_x,
                     curr_y + current_font_info->ascent, (char*)&c, 1);

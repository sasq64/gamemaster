...
static SF_bdffont* sBDXload(FILE* f, int* err, int* size, int MAXCHAR)
{
    ...
    for (i = 0; i < nprop; i++) {
        ...
        if ((p = starts(s, "DEFAULT_CHAR "))) defchar = atoi(p);
        if ((p = starts(s, "FONT_ASCENT "))) ascent = atoi(p);
        if ((p = starts(s, "FONT_DESCENT "))) descent = atoi(p);
    }
    for (;;) {
        fgets(s, 1024, f);
    ...
    po += k;
    if (fngot) free(fontname);
    k = strlen(copyright) + 1;
    memmove(po, copyright, k);
    po += k;
    if (cpgot) free(copyright);
    ...
    for (k = 0; k < nchars; k++) {
        ...
        for (;;) {
            ...
            if ((p = starts(s, "ENCODING "))) {
                i = atoi(p);
                i &= 0xffff;
                if (i <= MAXCHAR) font->glyphs[i - minch] = po - pbeg;
                hasenc = 1;
            }
...
{
    if (s) {
        if (s->data) return (int)((SF_bdffont*)(s->data))->ascent;
    }
    return 0;
}
...
{
    if (s) {
        if (s->data) return (int)((SF_bdffont*)(s->data))->descent;
    }
    return 0;
}
...
{
    if (s) {
        if (s->data) return (int)((SF_bdffont*)(s->data))->minchar;
    }
    return 0;
}
...
{
    if (s) {
        if (s->data) return (int)((SF_bdffont*)(s->data))->maxchar;
    }
    return 0;
}
...
static SFONT* myfonts[9] = {NULL, NULL, NULL, NULL, NULL,
                            NULL, NULL, NULL, NULL};
...
static int styleidx(int zfont, int style)
{
    ...
    if (z_header.flags & FIXED_FONT_FLAG) zfont = FIXED_WIDTH_FONT;
...
static SFONT* tryloadfont(char* fspec, SFONT* like)
{
    ...
    if (m_fontdir != NULL) fn = sf_searchfile(fspec, fnlen, buf, m_fontdir);

    for (;;) {
        p = strchr(fspec, '|');
        if (p) *p = 0;

        if (ttfontloader) b = ttfontloader(fspec, like, &err);
        if (!b) b = loadfont(fn, &err, &size);
        if (b) break;
        if (p) {
            *p = '|';
            fspec = p + 1;
...
void sf_initfonts(void)
{
    ...
    norm = SF_defaultfont;
    if (norm->minchar == 8192) {
        /* Need byteswap */
        zbyte* font = (zbyte*)SF_defaultfont;
        norm->minchar = font[4] | (font[5] << 8);
        norm->maxchar = font[6] | (font[7] << 8);
        norm->defchar = font[8] | (font[9] << 8);
        for (i = norm->minchar; i <= norm->maxchar; i++) {
            int m = (i - norm->minchar) * 4 + 12;
            norm->glyphs[i - norm->minchar] = font[m] | (font[m + 1] << 8);
        }
    }
    ...
    if (!m_vga_fonts) {
        for (i = 0; i <= 8; i++)
            if (m_fontfiles[i]) {
                SFONT* b = tryloadfont(
                    m_fontfiles[i], i == 8 ? myfonts[FIXED_WIDTH_FONT] : NULL);
                if (!b)
                    fprintf(stderr, "WARNING: could not load font%d [%s%c%s]\n",

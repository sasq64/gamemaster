...
{
    if (c >= ZC_ASCII_MIN && c <= ZC_ASCII_MAX) return true;
    if (c >= ZC_LATIN1_MIN && c <= ZC_LATIN1_MAX) return true;
    ...
    for (i = 0; i < h; i++) {
        ...
        if (ts->font->antialiased) {
            ...
            for (m = 0; m < nby; m++) {
                ...
                for (j = 0; j < 8; j++, t *= 2) {
                    if (xx < byw) {
                        if (t & 0x80) sf_wpixel(x + xx, y, color);
                    }
                    xx++;
                }
            }
...
{
    if (dirty) {
        SDL_UpdateTexture(texture, NULL, sbuffer, sbpitch * sizeof(zlong));
        myGrefresh();
        dirty = 0;
        return true;
    } else
...
static void sf_toggle_fullscreen(void)
{
    if (SDL_SetWindowFullscreen(
        ...
        if (!isfullscreen) SDL_SetWindowSize(window, AcWidth, AcHeight);
        myGrefresh();
    }
...
void sf_initvideo(int W, int H, int full)
{
    ...
    sprintf(banner, "SDL Frotz v%s - %s (z%d)", VERSION, f_setup.story_name,
            z_header.version);
    ...
    isfullscreen = full;
    if (full) video_flags |= SDL_WINDOW_FULLSCREEN_DESKTOP;
    if ((window = SDL_CreateWindow(banner, SDL_WINDOWPOS_UNDEFINED,
                                   SDL_WINDOWPOS_UNDEFINED, W, H, video_flags)))
        renderer = SDL_CreateRenderer(window, -1, 0);
    else
        renderer = NULL;
    if (renderer == NULL) {
        os_fatal("Couldn't create %dx%d window: %s", W, H, SDL_GetError());
    }
    SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "linear");
    if (SDL_RenderSetLogicalSize(renderer, W, H))
        os_fatal("Failed to set logical rendering size to %dx%d: %s", W, H,
                 SDL_GetError());
    ...
    if (!sbuffer) os_fatal("Could not create gc");
...
void os_draw_picture(int picture, int y, int x)
{
    ...
    if (pic->usespalette) {
        ...
        if (pic->adaptive) os_fatal("Adaptive images must be paletted");

        for (yy = 0; yy < eh; yy++) {
            for (xx = 0; xx < ew; xx++) {
                dst = sbuffer + x + (zlong)(xx * m_gfxScale_w) +
...
static zword decode_utf8(char* str)
{
    int i, n = 0;
    zword res;
    if (!(*str & 0200)) return *str;
    if (!(*str & 0100)) return 0;
    if (!(*str & 040)) {
        n = 2;
        res = *str & 037;
    } else if (!(*str & 020)) {
...
static zword goodzkey(SDL_Event* e, int allowed)
{
    ...
    case SDL_KEYDOWN:
        ...
        if (e->key.keysym.mod & (KMOD_LALT | KMOD_RALT)) {
            ...
            if (allowed) switch (e->key.keysym.sym) {
                case 'x':
                    return ZC_HKEY_QUIT;
                case 'p':
...
zword sf_read_key(int timeout, bool cursor, bool allowed, bool text)
{
    ...
    sf_flushtext();
    if (cursor) sf_drawcursor(true);
    sf_flushdisplay();

    if (timeout) mytimeout = sf_ticks() + m_timerinterval * timeout;
    if (text) SDL_StartTextInput();
    while (true) {
        /* Get the next input */
        while (SDL_PollEvent(&event)) {
            if ((inch = goodzkey(&event, allowed))) break;
        }
        if (inch) break;
    ...
    if (text) SDL_StopTextInput();

    if (cursor) sf_drawcursor(false);
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if ((c = goodzkey(&event, 1))) {
                ...
                case ZC_ARROW_RIGHT:
                    ...
                    if (pos < (int)mywcslen(buf)) pos++;
                    sf_DrawInput(buf, pos, ptx, pty, width, true);
                    continue;
                case ZC_ARROW_UP:
                case ZC_ARROW_DOWN:
                    if (searchpos < 0) searchpos = mywcslen(buf);
                    if ((c == ZC_ARROW_UP
                             ? gen_history_back
                             : gen_history_forward)(buf, searchpos, max)) {
                        pos = mywcslen(buf);
                        sf_DrawInput(buf, pos, ptx, pty, width, true);
                    }
                    continue;
...
zword os_read_mouse(void)
{
    ...
    if (c & SDL_BUTTON_LMASK) btn |= 1;
    if (c & SDL_BUTTON_RMASK) btn |= 2;
    if (c & SDL_BUTTON_MMASK) btn |= 4;
...
void os_tick(void)
{
    ...
    if (SDL_AtomicSet(&SFticked, 0)) {
        if (!sf_flushdisplay()) {
            SDL_Event ev;
            SDL_PumpEvents();
            while (SDL_PeepEvents(&ev, 1, SDL_GETEVENT, SDL_WINDOWEVENT,
                                  SDL_WINDOWEVENT) > 0)

...
static char* info[] = {"-a   watch attribute setting",
...
static char* info_footer = "\nError checking: 0 none, 1 first only (default), "
                           "2 all, 3 exit after any error.";
...
static char* footer = "More options and information are in the manual page.  "
                      "Type \"man sfrotz\".\n";
...
static void usage(int type)
{
    ...
    if (type == USAGE_NORMAL) puts(info_footer);
    puts(footer);
...
static void parse_options(int argc, char** argv)
{
    ...
    do {
        ...
        if (c == 0) {
            if (xscale_flag) m_xscale = atof(optarg);
            if (yscale_flag) m_yscale = atof(optarg);
        }
        if (c == '%') m_localfiles = true;
        if (c == 'a') f_setup.attribute_assignment = 1;
        if (c == 'A') f_setup.attribute_testing = 1;
        if (c == 'b') user_background = getcolor(optarg);
        if (c == 'B') option_scrollback_buffer = num;
        ...
        if (c == 'd') option_disable_color = 1;
        if (c == 'm') m_timerinterval = limit(num, 10, 1000000);
        if (c == 'N') user_names_format = copt;
        if (c == '@') m_reslist_file = optarg;
        if (c == 'f') user_foreground = getcolor(optarg);
        if (c == 'F') m_fullscreen = 1;
        ...
        if (c == 's') m_random_seed = num;
        if (c == 'S') f_setup.script_cols = num;
        if (c == 't') f_setup.tandy = 1;
        if (c == 'T') sf_osdialog = NULL;
        if (c == 'u') f_setup.undo_slots = num;
        if (c == 'v') print_version();
        if (c == 'V') m_vga_fonts = 1;
...
void os_process_arguments(int argc, char* argv[])
{
    ...
    if (argv[optind + 1] != NULL) f_setup.blorb_file = argv[optind + 1];
...
static const char* getdatename(const char* UNUSED(def), char* ext)
{
    ...
    strcpy(buf, f_setup.story_name);
    len = strlen(buf);
    sprintf(buf + len, "%04d%02d%02d%02d%02d%s", tm->tm_year + 1900,
            tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min, ext);
    return buf;
} /* getdatename */
                                 ...
                                 int flag)
{
...
#ifdef USE_UTF8
    {
        ...
        while (*zp) {
            if (*zp <= 0x7f) {
                if (i > FILENAME_MAX - EXT_LENGTH - 4) break;
                file_name[i++] = *zp;
            } else if (*zp > 0x7ff) {
                if (i > FILENAME_MAX - EXT_LENGTH - 6) break;
                file_name[i++] = 0xe0 | ((*zp >> 12) & 0xf);
                file_name[i++] = 0x80 | ((*zp >> 6) & 0x3f);
                file_name[i++] = 0x80 | (*zp & 0x3f);
            } else {
                if (i > FILENAME_MAX - EXT_LENGTH - 5) break;
    ...
    if (strchr(file_name, '.') == NULL) strcat(file_name, extension);

    /* Make sure it is safe to use this file name */
    result = TRUE;

    /* OK if the file is opened for reading */
    if (!newfile(flag)) goto finished;
...
#define plong(b)                                                               \
    (((int)((b)[3]) << 24) + ((int)((b)[2]) << 16) + ((int)((b)[1]) << 8) +    \
     (int)((b)[0]))

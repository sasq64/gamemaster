 ...
 */
void os_init_sound(void) {} /* os_init_sound */
...
static EFFECT* geteffect(int num)
{
    ...
    if (id == bb_ID_FORM) {
        result =
            getaiff(res.file, res.bbres.data.startpos, res.bbres.length, num);
    } else if (id == bb_ID_MOD || id == bb_ID_OGGV) {
        result =
            getmodule(res.file, res.bbres.data.startpos, res.bbres.length, num);
    }
    sf_freeresource(&res);

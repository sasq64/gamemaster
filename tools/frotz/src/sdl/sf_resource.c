...
{
    if (blorb_map) bb_destroy_map(blorb_map);
    blorb_map = NULL;
    if (blorb_fp) fclose(blorb_fp);
    blorb_fp = NULL;
...
int sf_load_resources(void)
{
    ...
    if (blorb_map) {
        ...
        if (bb_load_chunk_by_type(blorb_map, bb_method_Memory, &result,
                                  bb_ID_APal, 0) == bb_err_None) {
            m_adaptiveMode = TRUE;
...
static FILE* pathopen(const char* name, const char* path, const char* mode)
{
    ...
    while (*path) {
        ...
        while (*path && *path != OS_PATHSEP)
            *bp++ = *path++;
        if (*(path - 1) != OS_DIRSEP) *bp++ = OS_DIRSEP;
...
void sf_readsettings(void)
{
    ...
    if (m_fontdir != NULL && m_fontdir[0] != PATH_SEPARATOR) {
        ...
        m_fontdir_temp =
            malloc(((fontdir_len + homedir_len) * sizeof(char)) + 3);

        if ((m_fontdir[0] == '~') && (m_fontdir[1] == PATH_SEPARATOR))
            snprintf(m_fontdir_temp, fontdir_len + homedir_len + 3, "%s%c%s",
                     myhome, PATH_SEPARATOR, m_fontdir + 2);
        else
            snprintf(m_fontdir_temp, fontdir_len + homedir_len + 3, "%s%c%s",
                     myhome, PATH_SEPARATOR, m_fontdir);
...
zlong sf_GetColour(int colour)
{
    ...
    /* Default colours */
    if (colour == 16) return m_defaultFore;
    if (colour == 17) return m_defaultBack;
...
{
    if (m_IsInfocomV6) return sf_GetColour(fore ? WHITE_COLOUR : BLACK_COLOUR);
...
int sf_GetColourIndex(zlong colour)
{
    ...
    for (i = 0; i < 11; i++) {
        if (m_colours[i] == colour) return i + BLACK_COLOUR;
    }

    /* Is this a default colour? */
    if (m_defaultFore == colour) return 16;
    if (m_defaultBack == colour) return 17;

    /* Is this colour already in the table? */
    for (i = 0; i < NON_STD_COLS; i++) {
        if (m_nonStdColours[i] == colour) return i + 18;
    }
    ...
    while (index == -1) {
        ...
        m_nonStdIndex++;
        if (m_nonStdIndex >= NON_STD_COLS) m_nonStdIndex = 0;
    }
    return index;
} /* sf_GetColourIndex */
...
void os_init_screen(void)
{
    ...
    if (m_xscale > 0.0) m_gfxScale_w *= m_xscale;

    if (m_yscale > 0.0) m_gfxScale_h *= m_yscale;
    ...
    if (z_header.version >= V5) z_header.config |= CONFIG_COLOUR;
    ...
    z_header.screen_rows =
        (zbyte)(z_header.screen_height / z_header.font_height);

    /* Check for sound */
    if ((z_header.version == V3) && (z_header.flags & OLD_SOUND_FLAG)) {
        if (((blorb_map == NULL) && (m_localfiles == 0)) || (!sf_initsound()))
    ...
    if (z_header.version >= V5) {
        zword mask = 0;
        if (z_header.version == V6) mask |= TRANSPARENT_FLAG;
...
void sf_freeresource(myresource* res)
{
    ...
    if ((blorb_map) && (cnu >= 0)) bb_unload_chunk(blorb_map, cnu);
} /* sf_freeresource */
...
int sf_getresource(int num, int ispic, int method, myresource* res)
{
    ...
    if (st == bb_err_None) {
        ...
        if (method == bb_method_FilePos) res->file = blorb_fp;
    }
    return st;
} /* sf_getresource */

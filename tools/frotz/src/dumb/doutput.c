...
static char* bbcode_colour[] = {"black",     "red",     "green",  "yellow",
                                "blue",      "magenta", "cyan",   "white",
                                "lightgrey", "grey",    "dimgrey"};
...
static void show_cell_irc(cell_t cel)
{
    static char lastfg = DEFAULT_DUMB_COLOUR, lastbg = DEFAULT_DUMB_COLOUR,
                lastbold = 0, lastemph = 0;

    char fg = cel.fg, bg = cel.bg;
    ...
    if (fg != lastfg || bg != lastbg) {
        ...
        if (fg != DEFAULT_DUMB_COLOUR) {
            if (bg != DEFAULT_DUMB_COLOUR)
                ...
                printf("\003%s%u", ((unsigned char)fg < 10) ? "0" : "",
                       (unsigned char)fg);
        }
    }

    if (cel.style & BOLDFACE_STYLE) {
        if (!lastbold) putchar('\002');
        lastbold = 1;
    } else {
        if (lastbold) putchar('\002');
        lastbold = 0;
...
{
    static char lastfg = DEFAULT_DUMB_COLOUR, lastbg = DEFAULT_DUMB_COLOUR,
                lastbold = 0, lastemph = 0, lastrev = 0;
    ...
    if (cel.style & REVERSE_STYLE) {
        if (!lastrev) printf("\033[7m");
        lastrev = 1;
    } else {
        if (lastrev) printf("\033[27m");
        lastrev = 0;
    }

    if (cel.style & BOLDFACE_STYLE) {
        if (!lastbold) printf("\033[1m");
        lastbold = 1;
    } else {
        if (lastbold) printf("\033[22m");
        lastbold = 0;
    }

    if (cel.style & EMPHASIS_STYLE) {
        if (!lastemph) printf("\033[4m");
        lastemph = 1;
    } else {
        if (lastemph) printf("\033[24m");
        lastemph = 0;
    }
...
static void show_cell_bbcode(cell_t cel)
{
    ...
    if (lastfg != defaultfg &&
        (fix != lastfix || bg != lastbg || fg != lastfg)) {
        printf("[/color]");
        lastfg = defaultfg;
    }
...
{
#ifndef DISABLE_FORMATS
    if (f_setup.format != FORMAT_NORMAL) return FALSE;
#endif
    return (((c.style == PICTURE_STYLE) && !show_pictures) ||
            ((c.c == ' ') &&
             ((c.style != REVERSE_STYLE) || (*rv_blank_str == ' '))));
} /* will_print_blank */
...
static void show_line_prefix(int row, char c)
{
    if (show_line_numbers) {
        if (row == -1) {
            show_cell(
                make_cell(0, DEFAULT_DUMB_COLOUR, DEFAULT_DUMB_COLOUR, '.'));
            show_cell(
                make_cell(0, DEFAULT_DUMB_COLOUR, DEFAULT_DUMB_COLOUR, '.'));
        } else {
            char s[4];
            ...
            show_cell(
                make_cell(0, DEFAULT_DUMB_COLOUR, DEFAULT_DUMB_COLOUR, s[0]));
            show_cell(
                make_cell(0, DEFAULT_DUMB_COLOUR, DEFAULT_DUMB_COLOUR, s[1]));
        }
    }
    if (show_line_types)
...
static void show_row(int r)
{
    if (r == -1) {
        ...
        for (last = z_header.screen_cols - 1; last >= 0; last--) {
            if (!will_print_blank(dumb_row(r)[last])) break;
        }
...
static void dumb_set_cell(int row, int col, cell_t c)
{
    ...
    if (c.style == test.style && c.fg == test.fg && c.bg == test.bg &&
        c.c == test.c) {
        result = TRUE;
    }
...
{
    return ((c.c == ' ') || ((c.style == PICTURE_STYLE) && !show_pictures));
} /* is_blank */

static void dumb_copy_cell(int dest_row, int dest_col, int src_row, int src_col)
{
...
void os_erase_area(int top, int left, int bottom, int right, int UNUSED(win))
{
    ...
    for (row = top; row <= bottom; row++) {
        for (col = left; col <= right; col++)
            dumb_set_cell(
                row, col,
                make_cell(current_style, current_fg, current_bg, ' '));
    }
} /* os_erase_area */
...
void os_start_sample(int UNUSED(a), int UNUSED(b), int UNUSED(c),
                     zword UNUSED(d))
{
}
...
void dumb_set_picture_cell(int row, int col, zchar c)
{
    dumb_set_cell(
        row, col,
        make_cell(current_style | PICTURE_STYLE, current_fg, current_bg, c));
...
void dumb_display_user_input(char* s)
{
    ...
    while (*s)
        dumb_row(cursor_row)[cursor_col++] =
            make_cell(0, DEFAULT_DUMB_COLOUR, DEFAULT_DUMB_COLOUR, *s++);
...
void dumb_discard_old_input(int num_chars)
{
    ...
    os_erase_area(cursor_row + 1, cursor_col + 1, cursor_row + 1,
                  cursor_col + num_chars, -1);
...
void dumb_show_screen(bool show_cursor)
{
    ...
    for (r = hide_lines; r < z_header.screen_rows; r++) {
        for (c = 0; c < z_header.screen_cols; c++) {
            if (dumb_changes_row(r)[c] && !is_blank(dumb_row(r)[c])) break;
        }
    ...
    if (show_cursor && (cursor_row == last)) {
        for (c = cursor_col; c < z_header.screen_cols; c++) {
            if (!is_blank(dumb_row(last)[c])) break;
        }
        if (c == z_header.screen_cols) last--;
    }

    /* Display the appropriate rows.  */
    if (compression_mode == COMPRESSION_MAX) {
        for (r = first; r <= last; r++) {
            if (changed_rows[r]) show_row(r);
        }
    } else {
        /* COMPRESSION_SPANS */
        for (r = first; r <= last; r++) {
                                ...
                                bool startup)
{
    ...
    if (!strncmp(setting, "pb", 2)) {
        ...
        switch (setting[1]) {
        case 'm':
            compression_mode = COMPRESSION_MAX;
            break;
        case 's':
            compression_mode = COMPRESSION_SPANS;
            break;
        ...
        case 'n':
            ...
            break;
        case 'o':
            rv_mode = RV_DOUBLESTRIKE;
            break;
        case 'u':
            rv_mode = RV_UNDERLINE;
            break;
        case 'c':
            rv_mode = RV_CAPS;
            break;
        case 'b':
            strncpy(rv_blank_str, setting[2] ? &setting[2] : " ", 4);
            break;
        default:
            return FALSE;
        }
        ...
        for (q = (unsigned char*)&rv_blank_str[1]; *q; q++)
            if (*q < 0x80 || *q >= 0xc0 ||
                (unsigned char)rv_blank_str[0] < 0x80)
                *q = 0;
#else
        rv_blank_str[1] = 0;
#endif
        ...
        for (p = "sample reverse text"; *p; p++)
            show_cell(make_cell(REVERSE_STYLE, DEFAULT_DUMB_COLOUR,
                                DEFAULT_DUMB_COLOUR, *p));
        putchar('\n');
        for (i = 0; i < screen_cells; i++)

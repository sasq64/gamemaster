    ...
    for (;;) {
        ...
        if (!strcmp(command, "t")) {
            ...
            if (timeout) {
                ...
                time_t now = time(0);
                if (elapsed == 0) elapsed = (now - start_time) * 10 * speed;
                if (elapsed >= timeout) {
                    time_ahead = elapsed - timeout;
                    s[0] = '\0';
                    return TRUE;
            ...
            if (!do_more_prompts)
                ...
                for (;;) {
                    int i;
                    for (i = 0; (i < z_header.screen_rows - 2) && *next_page;
                         i++)
                    ...
                    printf("HELP: Type <return> for more, or q <return> to "
                           "stop: ");
                    fflush(stdout);
                    dumb_getline(s);
                    if (!strcmp(s, "q\n")) break;
...
zchar os_read_key(int timeout, bool show_cursor)
{
    ...
    if (timed_out) return ZC_TIME_OUT;
                   ...
                   int continued)
{
     ...
     * we're continuing.  */
    if (timed_out_last_time && !continued) read_line_buffer[0] = '\0';

    if (read_line_buffer[0] == '\0') {
        timed_out =
            dumb_read_line(read_line_buffer, NULL, TRUE, timeout,
...
char* os_read_file_name(const char* default_name, int flag)
{
    ...
    if ((flag == FILE_SAVE || flag == FILE_SAVE_AUX || flag == FILE_RECORD) &&
        ...
        if (tolower(fullpath[0]) != 'y') return NULL;
    }
    return file_name;
} /* os_read_file_name */
...
void dumb_init_input(void)
{
    ...
    if (z_header.version >= V5) z_header.flags &= ~(MOUSE_FLAG | MENU_FLAG);
} /* dumb_init_input */
